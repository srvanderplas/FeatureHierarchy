...
%         \comment{We might have to go up with the correlation a bit. I'm still worried that people will pick the cluster plot from the trend line lineup because of the lowest slope. }
    ...
    \item For the $K$ clusters, we want to have nearly equal sized groups, but uphold some variability. Group sizes are therefore determined as a draw from a multinomial distribution: determine group sizes $g = (g_1, ..., g_K)$, with $N = \sum_{i=1}^K g_i$, for clusters $1, ..., K$ as a random draw $g \sim \text{Multinomial}(K, p)$ where $p = \tilde{p}/\sum_{i=1}^K \tilde{p}$ for  $\tilde{p} \sim N(\frac{1}{K}, \frac{1}{2 K^2})$. 
...
\begin{algorithm}
  ...
  \end{enumerate}
\end{algorithm}
\comment{and again, I could imagine some examples here. }
...
require(ggplot2)
qplot(fline, data=res$nulls, binwidth=0.02, fill=I("grey70"), colour=I("grey20")) + geom_vline(aes(xintercept=res$trends), colour="black") + theme_bw()
ggsave("figure/fline.pdf", width=8, height=4)
qplot(fgroup, data=res$nulls, binwidth=0.01, fill=I("grey70"), colour=I("grey20")) + geom_vline(aes(xintercept=res$clusters), colour="black") + theme_bw()
