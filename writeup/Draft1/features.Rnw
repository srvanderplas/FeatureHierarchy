...
<<parameter-accuracy-rate, echo=F, fig.width=12, fig.height=5, out.width='\\linewidth', fig.cap="Identified targets for each level of parameter settings (across plot types).  \\label{fig:accuracy-parms}", dependson='results-setup'>>=
...
p1 <- qplot(cluster.diff2, accuracy, data=parm.accuracy, facets=k2~trend.diff2, size=I(3), colour=answer.type, shape=answer.type) + 
  theme_bw() + xlab("") + 
  geom_line(aes(group=answer.type)) + 
...
p2 <- qplot(trend.diff2, accuracy, data=parm.accuracy, facets=k2~cluster.diff2, size=I(3), colour=answer.type, shape=answer.type) + 
  theme_bw() + xlab("") + 
  geom_line(aes(group=answer.type)) + 
  scale_colour_brewer("Target Identified", palette="Set1") + 
  scale_shape_discrete("Target Identified") + 
