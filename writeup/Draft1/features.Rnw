...
\begin{algorithm}
  ...
  \begin{enumerate}
    \item Generate $x_i$, $i=1, ..., N$, a sequence of evenly spaced points from $[-1, 1]$ ($\sigma_T$ added and subtracted to match the range of cluster points in $x$) \comment{I don't understand the text in the parenthesis}
    \item Jitter $x_i$: $x_i = x_i + \eta_i$, $\eta_i \sim \text{Unif}(-z, z)$, $z = 1/5*(2/(N-1)$
    \comment{There's something going on with the parentheses: two opening, only one closing}
...
\comment{Include formula of correlation coefficient in the new notation here}


\comment{Include some examples here. }
...
%         \comment{We might have to go up with the correlation a bit. I'm still worried that people will pick the cluster plot from the trend line lineup because of the lowest slope. }
    ...
    \[
    g \sim \text{Multinomial}(K, p) \text{ where } p = \tilde{p}/\sum_{i=1}^K \tilde{p}_i,
    \]
    for  $\tilde{p} \sim N(\frac{1}{K}, \frac{1}{2 K^2})$. 
