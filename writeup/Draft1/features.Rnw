...
useranswers <- ddply(useranswers, .(ip_address, nick_name), transform, ntrials = length(unique(pic_id)), trial.no = rank(start_time))
...
From Figure~\ref{fig:targets} we see that users identified more cluster targets than trend targets, but users also do not primarily identify one target over another target, but generally pick both targets over the course of ten lineups.
\begin{figure}
\centering
...
time.model2 <- lmer(log(as.numeric(trial.time))~ simpleoutcome + simpleoutcome:plottype + first.trial  + trend.diff + cluster.diff  + (1|individualID) , data=modeldata)
...
@
\caption{\label{fig:timemodel} Results of a model describing log evaluation time by evaluation outcome and plot type. Participants take less time to evaluate plots with a single aesthetic compared with more complicated plots. }
\end{figure}
