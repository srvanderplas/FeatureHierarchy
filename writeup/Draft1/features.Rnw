...
extra.participant.data <- sum(modeldata$trial.no>10)
...
\comment{Here, we could add some discussion of the gini index.}
...
Additionally, all trials which took greater than 3 minutes to complete have also been removed; we have no guarantee that a participant was not multitasking during their trials, and 3 minutes is an excessive amount of time for lineup completion (only \Sexpr{round(mean(modeldata$trial.time>180)*100, 1)}\% of trials were more than 3 minutes long, and the median time to complete the entire experiment was \Sexpr{round(as.numeric(median(totaltime$total.experiment.time)))} minutes).
...
It really only looks impressive if we do cut out the trials more than 3 minutes long. Given that average time to complete the entire study was 8 minutes, that seems reasonable to me, but I understand where it gets dicey. 
}

\begin{figure}
<<timemodel, echo=FALSE, fig.width=10, fig.height=5, out.width='\\textwidth', dependson='results-setup'>>=
library(lubridate)

# modeldata$time_taken <- as.numeric(ymd_hms(modeldata$end_time) - ymd_hms(modeldata$start_time))
time.model <- lmer(log(as.numeric(trial.time))~ simpleoutcome + simpleoutcome:plottype + first.trial  + (1|individualID) + (1|dataset), data=modeldata)


coefs <- data.frame(exp(confint(time.model, method="Wald")))[-(1:5),]
names(coefs) <- c("low", "high")
coefs$estimate <- exp(fixef(time.model)[-(1:5)])
coefs$name <- names(fixef(time.model))[-(1:5)]
...
# 
# coefs2 <- data.frame(exp(confint(time.model, method="Wald")))[2:10,]
# names(coefs2) <- c("low", "high")
# coefs2$estimate <- exp(fixef(time.model)[2:10])
# coefs2$name <- names(fixef(time.model))[2:10]
# coefs2$outcome <-  "neither"
# coefs2$plottype <- gsub("plottype", "", gsub("\\(Intercept\\)", "plain", coefs2$name))
# coefs2$simple <- coefs2$outcome %in% c("both")
# coefs2$simple <- factor(coefs2$simple)
# levels(coefs2$simple) <- c("line, group, neither", "both")
# 
# coefs <- rbind(coefs, coefs2)
# coefs$outcome <- factor(coefs$outcome, levels=c("neither", "trend", "cluster", "both"))
# # coefs$pos <- as.numeric(factor(coefs$plottype)) + (as.numeric(factor(coefs$outcome))-2.5)/6
# coefs$pos <- as.numeric(factor(coefs$plottype)) + (as.numeric(factor(coefs$simple))-2)/4

# ggplot(data=coefs, aes(x=pos, y=estimate, ymin=low, ymax=high, colour=simple)) + 
#   geom_pointrange() + 
#   scale_colour_brewer(palette="Set1", "Targets\nIdentified") +
#   coord_flip() + 
#   scale_x_continuous("", breaks=1:10, labels=levels(coefs$plottype)) + 
#   annotate(geom="segment", x=0:10+0.5, xend=0:10+0.5, y=-Inf, yend=Inf, color="grey50") + 
#   theme_bw() + 
#   ylab("Estimate of the (additional) time taken in seconds for each plot type given outcome")
...
qplot(estimate, pos, data=coefs, colour=outcome, size=I(4), shape=outcome) + theme_bw() + 
  ...
  scale_y_continuous("", breaks=1:9, labels=levels(coefs$plottype)) + 
  xlab("Estimate of the (additional) time taken in seconds for each plot type given outcome") + ylab("")
@
...
\subsection{Participant Reasoning}
\comment{Short lexical analysis to see if group/line was mentioned in reasoning?}
...
\newtext{This approach is similar to that taken in \citet{niladri:2014}}

\newtext{
The trend strength is assessed numerically using the coefficient of determination, $R^2$, which is calculated as 
}

\begin{align}
R^2 & = & \left(\sum_{1 \leq i \leq N} (\hat{y}_i-\overline{y}_i)^2\right)/\left(\sum_{1\leq i\leq N} (y_i - \overline{y}_i)^2\right)
\end{align}

\newtext{
The cluster strength, $C^2$ is assessed by comparing the percent of variation in x and y accounted for by the clusters to the total variation. For points $(x_i, y_i)$ define $z_i = \{1, ..., K\}$ to index cluster membership. 
}


\begin{align}
C^2 & = & 
\left(\sum_{1\leq i \leq N} 
\left[(x_i - \overline{x}_{z_i} )^2 + (y_i - \overline{y}_{z_i})^2\right]
\right)/\left(\sum_{1\leq i \leq N} 
\left[(\overline{x}_{i} - \overline{x})^2 + (\overline{y}_{i} - \overline{y})^2
\right]\right)
\end{align}
...
Figures \ref{fig:simulationLineIntervals} and \ref{fig:simulationClusterIntervals} show the 25th and 75th percentiles of the distribution of $R^2$ and $C^2$ summary statistics for each set of parameter values. These plots guided our evaluation of ``easy", ``medium" and ``hard" parameter values for line and cluster tasks. 
...
@
%' <<time-model, echo=FALSE, fig.width=8, fig.height=4, out.width='.75\\linewidth'>>=
%' timemodel <- lmer(as.numeric(trial.time) ~ plottype + (1|dataset), data=subset(modeldata, as.numeric(trial.time)<3*60)) # 
%' timemodel.ci <- data.frame(Estimate=fixef(timemodel), confint(timemodel, method="Wald"))
...
%'   str_replace(fixed("Trend + :"), "Trend:") %>%
%'   str_replace("Ellipse", "Ellipse + ") %>%
%'   str_replace("Error", "Error + ") %>%
%'   str_replace("( \\+ )$", "")
%' timemodel.ci$label <- factor(timemodel.ci$label, levels=timemodel.ci$label[order(timemodel.ci$Estimate)])
%' qplot(data=timemodel.ci[-c(1:6),], x=label, y=Estimate, ymin=LB, ymax=UB, geom="pointrange") + coord_flip() + ylab("Average Additional Time in Seconds To Identify Target Plot") + xlab("") + theme_bw()
%' @
%' 


\comment{I feel like this should be in here, but it's not that clear what the significance is...? Any model is just going to be really confusing, in my opinion. }

\comment{OK, I agree, that we should be talking about the parameters at some place, but in terms of accuracy we are not going to get anywhere, because the results are, as you say, pretty confusing. So let's leave this for later and only include the parameter analysis for the speed and confidence analysis. }
...
p1 <- qplot(cluster.diff2, accuracy, data=parm.accuracy, facets=k2~trend.diff2, colour=answer.type, linetype=answer.type) + 
  ...
  scale_linetype_manual("Target Identified", values=c(1,3, 4)) + 
...
<<parameters-time,echo=FALSE, fig.cap="There is no evidence that parameter settings significantly impacted time to completion. \\label{fig:parameterTime}", fig.width=8, fig.height=4, out.width='.75\\linewidth'>>=
par.time <- lmer(log(as.numeric(trial.time)) ~ trend.diff*cluster.diff*k + (1|individualID) + (1|dataset), data=modeldata)

par.time.fixef <- data.frame(estimate=fixef(par.time), confint(par.time, method="Wald"))
names(par.time.fixef)[2:3] <- c("LB", "UB")
par.time.fixef$label <- rownames(par.time.fixef) %>%
  str_replace_all(fixed(":"), " x ") %>%
  str_replace_all(fixed("trend.diff"), "Trend:") %>% 
  str_replace_all(fixed("cluster.diff"), "Cluster:") %>%
  str_replace_all("k", "K:") %>%
  str_replace_all("medium", "Medium") %>% 
  str_replace_all("hard", "Hard")
par.time.fixef$label <- factor(par.time.fixef$label, levels=par.time.fixef$label)

ggplot(data=par.time.fixef[-1,]) + 
  geom_vline(aes(xintercept=1), color="grey") + 
  geom_segment(aes(x=exp(LB), xend=exp(UB), y=label, yend=label)) + 
  geom_point(aes(x=exp(estimate), y=label)) + 
  theme_bw() + 
  xlab("Additional estimated time (in seconds) to evaluate lineups\n based on parameter settings") + 
  ylab("") + 
  ggtitle("Lineup evaluation time and parameter settings")
@

\comment{I know that confidence level is discrete, but as a quick-and-dirty approach, here's a linear model describing confidence level as a function of parameter settings.}
<<parameter-conf, echo=FALSE, fig.cap="There is no evidence that parameter settings significantly impacted participant confidence. \\label{fig:parameterTime}", fig.width=8, fig.height=4, out.width='.75\\linewidth'>>=
conf.model <- lmer(conf_level ~ trend.diff*cluster.diff*k + (1|individualID) + (1|dataset), data=modeldata)

conf.model.fixef <- data.frame(estimate=fixef(conf.model), confint(conf.model, method="Wald"))
names(conf.model.fixef)[2:3] <- c("LB", "UB")
conf.model.fixef$label <- rownames(conf.model.fixef) %>%
  str_replace_all(fixed(":"), " x ") %>%
  str_replace_all(fixed("trend.diff"), "Trend:") %>% 
  str_replace_all(fixed("cluster.diff"), "Cluster:") %>%
  str_replace_all("k", "K:") %>%
  str_replace_all("medium", "Medium") %>% 
  str_replace_all("hard", "Hard")
conf.model.fixef$label <- factor(conf.model.fixef$label, levels=conf.model.fixef$label)

ggplot(data=conf.model.fixef[-1,]) + 
  geom_vline(aes(xintercept=0), color="grey") + 
  geom_segment(aes(x=LB, xend=UB, y=label, yend=label)) + 
  geom_point(aes(x=estimate, y=label)) + 
  theme_bw() + 
  xlab("Additional estimated confidence in lineup evaluation\n based on parameter settings") + 
  ylab("") + 
  ggtitle("Participant confidence and parameter settings")
@
