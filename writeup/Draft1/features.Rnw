...
\newtext{For two numeric variables $x$ and $y$ and grouping variable $g$ with $g_i \in \{1, ..., K\}, i = 1, ..., n$, we compute the  {\it cluster index} as follows:
for each level of $g$, find  mean $\bar{x}_j$ and  $\bar{y}_j$ for $j = 1, ... K$.
