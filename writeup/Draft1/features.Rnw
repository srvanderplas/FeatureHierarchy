...
cis$name <- gsub("plottype", "", names(fixef(m1)))
...
cis2 <- cis[-1,]
cis2$name <- with(cis2, reorder(name, estimate))
qplot(exp(estimate), name, data=cis2) + theme_bw() + geom_vline(xintercept=1, colour="grey70") + 
  geom_segment(aes(x=exp(X2.5..), xend=exp(X97.5..), y=name, yend=name)) + 
  geom_text(aes(x=0, y=name, label=letters), data=cis2) +
  ggtitle("Odds of Group vs Line given one target was identified")
