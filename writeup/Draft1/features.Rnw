...
Discussion of preattentive visual features \citep{healey:preattentive}
...
& Color + Shape, Color + Ellipse\\\hline
\multirow{2}{*}{Trend} & Line, Error band \\
& Line + Error band\\\hline
Conflict & Color + Trend Line, Color + Trend Line + Error band\\\hline\hline
\end{tabular}
\end{center}
