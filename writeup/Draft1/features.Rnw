...
\citet{healey1999large} extended this work, demonstrating that certain features of three-dimensional data displays are processed preattentively, but that \newtext{neither target identification nor three-dimensional data processing} always translate into faster or more accurate inference about the data displayed, particularly when participants must integrate several preattentive features to understand the data. 
...
\comment{I like the chart, but it's not ours - let's replace it with scatterplots and point out Gestalt principles in them.}

\begin{figure}
<<gestalt1, echo =FALSE, out.width='0.3\\textwidth', fig.width=3, fig.height=3>>=
...
@
<<gestalt2, echo =FALSE, out.width='0.3\\textwidth', fig.width=3, fig.height=3>>=
x <- rnorm(60)
y <- rnorm(60)
group <- rep(1:4, length=60)
qplot(x,y, colour=factor(group%/%2), shape=factor(group%/%2)) + theme_bw() + xlab("x") + ylab("y") + scale_colour_brewer(palette="Set1") + theme(legend.position="none")
@
<<gestalt3, echo =FALSE, out.width='0.3\\textwidth', fig.width=3, fig.height=3>>=
x1 <- runif(50,-.75,1.25)
...
The generative model for null data  is created as a mixture model $M_0$ that draws $n_c \sim B_{N, \lambda}$ observations from the cluster model, and $n_T = N - n_c$ from the regression model $M_T$. Observations are assigned groups using k-means clustering, where the clustering is repeated until all $K$ clusters contain at least $N/(K*2+1)$ points; this condition ensures that clusters will not attract participant attention for being too ``uneven" and thus bias the results. 
% 
...
\item Color: points in each of the panels are colored based on the results of k-means clustering. 
