...
% \comment{Extend this to a roadmap for the rest of the paper}
We will discuss the experimental design, model considerations, and graphical features compared experimentally in section \ref{sec:ExperimentalDesign}. 
...
for each data set generated, statistics describing the trend strength and cluster strength were computed. We compared statistics for the relevant target plot to the most extreme value for the 18 null plots - for instance, the trend target plot $R^2$ value was compared to the maximum $R^2$ value in the 18 null plots. These evaluations provide an estimate of the difficulty of identifying the target plot numerically; a target plot with $R^2=0.95$ would be very easy to identify when surrounded by null plots with $R^2=0.5$. Graphical summaries of simulation results are provided in appendix \ref{app:parametersimulation}.
}
...
\newtext{
Participants were recruited using Amazon's Mechanical Turk service, which connects interested workers with ``Human Intelligence Tasks" (HITs), which are (typically) short tasks which cannot be easily automated. Workers were able to complete the task if they had previously attempted at least 100 HITs with a 95\% successful completion rate. These restrictions reduce the amount of data cleaning required by ensuring that participants have experience with the Mechanical Turk system. 
}

\newtext{
Participants were asked to complete an example task similar to the task in the experiment before deciding whether or not to complete the HIT. The lineups used as examples contained only one target (5 trend and 5 cluster trials were provided), and we asked that participants correctly identify target plots in at least two lineups before accepting the HIT and proceeding to the experimental phase. The webpage used to collect data from Amazon Turk participants is avilable at \url{http://www.mlcape.com:8080/mahbub/turk16/index.html}. No data was recorded from the example task because participants had not yet provided informed consent. 
}

\newtext{
Once participants completed the example task and provided informed consent, they could accept the HIT through Amazon and were directed to the main experimental task. Participants were asked to complete 10 lineups, answering ``Which plot is the most different from the others?". Participants were also asked to provide a short summary of their reasoning, such as ``Strong linear trend" or ``Groups of points", and to rate their confidence in their selection from 1 (least confident) to 5 (most confident). After the first question, participants were asked to provide basic demographic information: age range, gender, and highest level of education. Upon completion of 10 lineups, participants were provided with a text code to enter into Amazon Turk in order to receive the advertised payment of \$1.00. Participants took (on average) 8 minutes to complete the HIT (not including the required example task).
}
...
useranswers <- ddply(useranswers, .(ip_address, nick_name), transform, ntrials = length(unique(pic_id)), trial.no = order(start_time, decreasing=F))


modeldata <- useranswers[,c(1, 2, 9:29, 3:8)]
...
message(paste0(sum(modeldata$trial.no>10), " trials removed because participant >10 trials."))
modeldata <- filter(modeldata, trial.no<=10)
...
\comment{I don't know where the demographic information for the experiment is stored... HH: it's on turk16 now.}

\comment{We need to clean out a couple of these answers. Our study was built around haveing people evaluate ten lineups. That is what I'd like the data to be cut to - kick out all individuals with fewer than ten lineups and only take the first ten evaluations from participants. Keep track at each step of how many evaluations that is costing us ... 

1060 trials (participant completed <10 trials total)\\
422 trials (participant completed >10 trials)}

\comment{It would also be interesting to see, if individuals picked mostly lines or mostly groups or whether there was a mix of both. }
...
during which time \Sexpr{length(unique(modeldata$individualID))} individuals completed \Sexpr{nrow(modeldata)} unique lineup evaluations. Data from participants who completed fewer than 10 lineups was removed from the study, and trials in excess of 10 trials were also removed from the study. 
...
We  first consider the effect of plot aesthetics on target selection for each target type (separately), and then we will analyze the effect of parameter values on participant performance.

\comment{The two models (1) and (2) have the exact same shape, so just discuss the structure of the model first, and then report the results for the two different dependent variables in the subsections.}
...
We  model the probability of selecting the linear target plot as a function of plot type, with random effects for dataset (which encompasses parameter effects) and participant (accounting for variation in individual skill level). For plot type $i = 1, ..., 10$ displaying dataset $j=1, ..., 54$ by participant $k=1, ..., P$, 
...
<<line-ranef,echo=FALSE, dependson='line-model', fig.width=4, fig.height=4, fig.keep='all', fig.show='hold', out.width='.45\\linewidth'>>=
...
line.ranef.individual$type="Participant Effects"

line.ranef.data <- as.data.frame(ranef(line.model)[[2]])
names(line.ranef.data) <- "value"
line.ranef.data$id <- rownames(line.ranef.data)
line.ranef.data$type="Dataset Effects"

line.resid <- data.frame(id=modeldata$response.id, value=resid(line.model), type="Residuals")

line.random.effects <- rbind(line.ranef.data, line.ranef.individual, line.resid)

ggplot(data=line.random.effects) + 
...
library(arm)
binnedplot(fitted(line.model), resid(line.model))
...
HH: there's no reason to be concerned. your dependent variable is 0/1, that's why the residuals don't look normal. The random effects are not normal because theory tells us that they are only asymptotically normal, and there's too much grouping here for things to actually look normal. Just get rid of these residual plots, things are fine. We'll stick to interpreting the estimates.}

We  examine the probability of selecting the group target plot as a function of plot type, with random effects for dataset (which encompasses parameter effects) and participant (accounting for variation in individual skill level). For plot type $i = 1, ..., 10$ displaying dataset $j=1, ..., 54$ by participant $k=1, ..., P$, 


\newdo{Discuss variance/covariance and random effects?? HH: Other than we need them? No, I don't think that will be necessary}
...
The model fit here is the same as that shown in equation \eqref{eqn:linearModel}, except that success in this model is defined as identification of the cluster target plot. 
}
...
<<group-ranef,echo=FALSE, dependson='group-model', fig.width=4, fig.height=4, fig.keep='all', fig.show='hold', out.width='.45\\linewidth'>>=
group.ranef.individual <- as.data.frame(ranef(group.model)[[1]])
names(group.ranef.individual) <- "value"
group.ranef.individual$id <- rownames(group.ranef.individual)
group.ranef.individual$type="Participant Effects"

group.ranef.data <- as.data.frame(ranef(group.model)[[2]])
names(group.ranef.data) <- "value"
group.ranef.data$id <- rownames(group.ranef.data)
group.ranef.data$type="Dataset Effects"

group.resid <- data.frame(id=modeldata$response.id, value=resid(group.model), type="Residuals")

group.random.effects <- rbind(group.ranef.data, group.ranef.individual, group.resid)

ggplot(data=group.random.effects) + 
  geom_density(aes(x=value, fill=type)) + 
  facet_grid(.~type) + 
  scale_fill_discrete(guide="none")

library(arm)
binnedplot(fitted(group.model), resid(group.model))
...
\comment{
My take on this: HH: I like it, let's keep this!
...
When (trend + error) are present in the same plot, you get additional gestalt ordering principle(s): common region + good continuation \textbf{+ connectedness + closure}:\\
...
<<groupVline-ranef,echo=F,out.width='.45\\linewidth', fig.show='hold'>>=
gvl.ranef.data <- as.data.frame(ranef(gvl.model)[[1]])
names(gvl.ranef.data) <- "value"
gvl.ranef.data$id <- rownames(gvl.ranef.data)
gvl.ranef.data$type="Dataset Effects"

gvl.resid <- data.frame(id=designs$dataset, value=resid(gvl.model), type="Residuals")

gvl.random.effects <- rbind(gvl.ranef.data, gvl.resid)

ggplot(data=gvl.random.effects) + 
  geom_density(aes(x=value, fill=type)) + 
  facet_grid(.~type) + 
  scale_fill_discrete(guide="none")

library(arm)
binnedplot(fitted(gvl.model), resid(gvl.model))
@
...
\comment{The setup for this simulation is similar to what Niladri is doing in \citet{niladri:2014}.}
...
<<parameter-accuracy-rate, echo=F, fig.width=12, fig.height=6.5, out.width='\\linewidth', fig.cap="Identified targets for each level of parameter settings (across plot types).  \\label{fig:accuracy-parms}", dependson='results-setup'>>=
...
parm.accuracy$cluster.diff2 <- factor(parm.accuracy$cluster.diff2, levels=c("Cluster: Easy", "Cluster: Medium", "Cluster: Hard"))
...
p1 <- qplot(cluster.diff2, accuracy, data=parm.accuracy, facets=k2~trend.diff2, colour=answer.type, linetype=answer.type) + 
  theme_bw() + xlab("") + 
  geom_line(aes(group=answer.type), size=1) + 
  scale_y_continuous("Accuracy", limits=c(0,1)) + 
  scale_colour_brewer("Target Identified", palette="Set1") + 
  scale_linetype_manual("Target Identified", values=c(1,2, 4)) + 
  # scale_size_manual("Target Identified", values=c(1, 6, 3)) +
  theme(legend.position="bottom", axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5)) 
  
p2 <- qplot(trend.diff2, accuracy, data=parm.accuracy, facets=k2~cluster.diff2, colour=answer.type, linetype=answer.type) + 
  theme_bw() + xlab("") + 
  geom_line(aes(group=answer.type), size=1) + 
  scale_y_continuous("Accuracy", limits=c(0,1)) + 
  scale_colour_brewer("Target Identified", palette="Set1") + 
...
As data collection was conducted entirely online, we cannot measure responses in the millisecond range characteristic of many psychometric studies, however, the data server does record the time between  trial start and end time. \comment{it is not clear, how the `trial' is defined. it might be better to re-phrase this in terms of the time between presenting the lineup to the participant until the participant submits his/her answer. }
...
\comment{No, don't select based on the value of the dependent variable. Use a log transform instead. Most likely, the time taken by participants is close to a Gamma distribution. This is what we have used in the past to model this kind of data - have a look at Mahbub's paper (the arxiv one on Human Factors). Instead of average trial ties, use a robust approach, such as the median - that will give you almost the same answers, see figure~\ref{fig:timemodel}. There is no indication that the plot type matters for the time taken, even when we take the outcome of that viewing into account. What matters, is whether the lineup is the participants first lineup.  }
...
\begin{figure}
<<timemodel, echo=FALSE, fig.width=10, fig.height=5, out.width='\\textwidth'>>=
library(lubridate)
...
modeldata <- ddply(modeldata, .(individualID), transform, first = start==min(start))
modeldata$simple <- modeldata$outcome
idx <- grep("gini", modeldata$simple)
modeldata$simple[idx] <- "neither"

time.model <- lmer(log(time_taken)~ simple:plottype +first  + (1|individualID) + (1|data_name), data=modeldata)
coefs <- data.frame(exp(confint(time.model, method="Wald")))[-(1:2),]
names(coefs) <- c("low", "high")
coefs$estimate <- exp(fixef(time.model)[-(1:2)])
coefs$name <- names(fixef(time.model))[-(1:2)]
ll <- ldply(strsplit(coefs$name, split=":"), function(x) x)
coefs$outcome <- gsub("simple", "", ll$V1)
coefs$plottype <- gsub("plottype", "", ll$V2)
coefs$simple <- coefs$outcome %in% c("both")
coefs$simple <- factor(coefs$simple)
levels(coefs$simple) <- c("line, group, neither", "both")
coefs$plottype <- with(coefs, reorder(plottype, estimate))
qplot(estimate, plottype, data=coefs, colour=outcome, size=I(4), shape=outcome) + theme_bw() + 
  scale_colour_brewer(palette="Set1") +
  geom_segment(aes(x=low, xend=high, y=plottype, yend=plottype)) + facet_wrap(~simple) + xlab("Estimate of the (additional) time taken in seconds for each plot type given outcome") + ylab("")
@
\caption{\label{fig:timemodel} XXX figure would need some additional work, in case we want to keep it.}
\end{figure}
