...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item Generate cluster centers $(c_{xi}, c_{yi}), i=1, ..., K$:
      \begin{enumerate}
        \item Generate $c_{x}$ a permutation of $\{1, ..., K\}$, $c_{y}$ a permutation of $\{1, ..., K\}$
        \item if $.25 < \text{Cor}(c_{x}, c_{y}) < .9$ keep $c_{x},c_{y}$, otherwise, re-generate.
      \end{enumerate}
      \item Scale cluster centers; mean and SD are constant for a given choice of $K$: $\overline{c} = K(K+1)/2$, $Var(c) = \frac{K(K+1)(2K+1)}{6} - \frac{K^2(K+1)^2}{4}$
%       \begin{enumerate}
%         \item $c_{xi} = (c_{xi} - \overline{c}_x)/sd_{c_{x}}$
%         \item $c_{yi} = (c_{yi} - \overline{c}_y)/sd_{c_{y}}$
%       \end{enumerate}
    \item Determine group size: $g \sim Multinomial(K, p)$ where $p = p_1/\sum_1^K p_{1i}$ where  $p_{1i} \sim N(\frac{1}{K}, \frac{1}{2 K^2})$ 
    \item Generate points around cluster centers: 
      \begin{enumerate}
        \item $x^\ast_i = c_{xg_i} + e$, $e_i \sim N(0, q)$
        \item $y^\ast_i = c_{yg_i} + e$, $e_i \sim N(0, q)$
      \end{enumerate}
