...
\section{Setup for Lineups}
I would consider the values $q = 0.3, .35, .4, .45$ for $K = 3$ clusters to be interesting. 
We need to re-express the values of $q$ in terms of the standard deviation $s_C$, because the absolute values of $q$ only make sense for the scaled values. 
