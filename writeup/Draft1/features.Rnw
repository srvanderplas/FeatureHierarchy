...
time.model2 <- lmer(log(as.numeric(trial.time))~ simpleoutcome:plottype + first.trial  + trend.diff:cluster.diff:k  + (1|individualID) , data=modeldata)
...
time.model2 <- lmer(log(as.numeric(trial.time))~ first.trial  + simpleoutcome:trend.diff:cluster.diff:k  + (1|individualID) , data=modeldata)

coefs <- data.frame(exp(confint(time.model2, method="Wald")))[-c(1:2),]
names(coefs) <- c("low", "high")
coefs$estimate <- exp(fixef(time.model2)[-c(1:2)])
coefs$name <- names(fixef(time.model2))[-c(1:2)]
ll <- ldply(strsplit(coefs$name, split=":"), function(x) x)
coefs$outcome <- gsub("simpleoutcome", "", ll$V1)
...
# coefs$trend.diff <- with(coefs, reorder(trend.diff, estimate))

coefs$outcome <- factor(coefs$outcome, levels=c("neither", "cluster", "trend", "both"))
coefs$pos <- as.numeric(factor(coefs$k)) + (as.numeric(factor(coefs$outcome))-2.5)/5

qplot(estimate, pos, data=subset(coefs, outcome!="both"), colour=outcome, size=I(4), shape=outcome) + theme_bw() + 
  scale_colour_brewer(palette="Set1") +
  geom_segment(aes(x=low, xend=high, y=pos, yend=pos)) + 
  facet_grid(trend.diff~cluster.diff, scales="free_x") + 
