...
Unfortunately, this limits the choice to the set used in the Tableau software. In order to produce experimental stimuli accessible to the approximately 4\% of the population with red-green colorblindness \citep{colorvision}, we removed the grey hue from the palette. This modification produced maximally different color combinations which did not include red-green combinations, while also removing a color (grey) which is difficult to distinguish for those with color deficiency.  
...
HH: I'm completely fine with leaving things as is - I just didn't remember that excluding grey had this effect.}
...
We compute the correlation coefficient for all of the plots to assess the amount of linearity in each panel.  \newtext{As a measure of clustering we use a similar coefficient: here, we assess the amount of variability taken care of by including the grouping variable, compared to total variability. Note that for the purpose of clustering, variability is measured as the variability in both $x$ and $y$ from a common mean, i.e. we assume that the values in $x$ and $y$ are on the same scale (which we can easily achieve by an appropriate re-scaling).  } 
...
HH: ok, then let's just leave the descitption - and the construction as is - we could note somewhere that we needed to make sure to use the full range of possible values.
}
...
$K$ & \# Clusters & 3, 5 \\
$N$ & \# Points & $15\cdot K$ \\
$\sigma_T$ & Scatter around trend line &  .35, .45, .55\\
$\sigma_C$ & Scatter around cluster centers &  .25, 0.30, .35, 0.40\\\hline
\end{tabular}
\end{center}
...
require(ggplot2)
qplot(fline, data=res$nulls, binwidth=0.02) + geom_vline(aes(xintercept=res$trends), colour="red") + theme_bw()
ggsave("figure/fline.pdf", width=8, height=4)
qplot(fgroup, data=res$nulls, binwidth=0.01) + geom_vline(aes(xintercept=res$clusters), colour="red") + theme_bw()
...
\caption{\label{fig:fline}Histogram of $R^2$ values from \Sexpr{N} data sets of the null model ($K=3, N=45, \sigma_C=$\Sexpr{sC}, $\sigma_T= $\Sexpr{sT}). The lines in red are $R^2$ values of ten sample data sets from the Trend model $M_T$. }
\end{figure}

\begin{figure}[h]
\includegraphics[width=\textwidth]{figure/fgroup.pdf}
\caption{\label{fig:fgroup}Histogram of the distribution of the cluster measure based on \Sexpr{N} data sets from the null model ($K=3, N=45, \sigma_C= $\Sexpr{sC}, $\sigma_T= $\Sexpr{sT}). The lines in red correspond to the cluster measure from ten sample data sets from the Clustering model $M_C$. }
\end{figure}
...
\multirow{11}{*}{Dataset}
      ...
      $\sum_{jk} (2)(r)(10P)(\overline{y}_{\cdot jk\cdot\cdot\cdot}-
                               ...
                               \overline{y}_{\cdot\cdot\cdot\cdot\cdot\cdot})^2$ \\
      \cline{2-5}
      &    & \multirow{2}{*}{$(\alpha\beta\gamma)$} & \multirow{2}{*}{3} & 
