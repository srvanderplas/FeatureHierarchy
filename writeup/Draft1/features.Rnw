...
\begin{figure}[ht]
<<echo=FALSE, fig.width=7.5, fig.height=5, fig.keep='all', fig.show='hold', out.width='.49\\textwidth'>>=
...
qplot(age, data=users) + theme_bw() + xlab("Age of participants")

qplot(education, data=users) + theme_bw() + xlab("(self-reported) Highest level of participants' education")
@
...
>>>>>>> a756f4447f88df83264450ea9189dcbbad5fa234
