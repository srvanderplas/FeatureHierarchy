...
<<model-parameters>>=
modeldata$correct <- with(modeldata, both.correct | cluster.correct | trend.correct)
modeldata$sd.line <- factor(modeldata$sd.line)
modeldata$sd.cluster <- factor(modeldata$sd.cluster)
modeldata$k <- factor(modeldata$k)

model.param2 <- glmer(correct ~ k+ sd.line*k + sd.cluster*k + (1|individualID) + (1|data_name), data=modeldata, family=binomial(),  control=glmerControl(optimizer="bobyqa"))
