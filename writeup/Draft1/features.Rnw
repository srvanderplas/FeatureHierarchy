...
\newtext{Figure \ref{fig:accuracy-parms} shows aggregate }
<<parameter-accuracy-rate, echo=F, fig.width=10, fig.height=5, out.width='.8\\linewidth', fig.cap="Identified targets for each level of parameter settings (across plot types).  \\label{fig:accuracy-parms}", dependson='results-setup'>>=
...
parm.accuracy$answer.type <- factor(parm.accuracy$answer.type, levels=c("trend", "cluster", "neither"))
k.3 <- qplot(x=answer.type, y=accuracy, fill=answer.type, 
             geom="bar", data=subset(parm.accuracy, k==3), position="dodge", stat="identity") +
  facet_grid(trend.diff2~cluster.diff2) + 
  scale_fill_brewer(palette="Set1", guide="none") + 
  theme_bw() + 
  xlab("Target Identified") + 
  ylab("Proportion of Trials") + 
  ggtitle("K=3 Clusters")
k.5 <- qplot(x=answer.type, y=accuracy, fill=answer.type, 
             geom="bar", data=subset(parm.accuracy, k==5), position="dodge", stat="identity") +
  facet_grid(trend.diff2~cluster.diff2) + 
  scale_fill_brewer(palette="Set1", guide="none") + 
  theme_bw() + 
  xlab("Target Identified") + 
  ylab("Proportion of Trials") + 
  ggtitle("K=5 Clusters")

grid.arrange(k.3, k.5, nrow=1)
@

<<plot-accuracy-rate, echo=F, fig.width=8, fig.height=4, out.width='.8\\linewidth', fig.cap="Proportion of trials identifying each target for each plot type (across parameter settings).\\label{fig:accuracy-plottype}", dependson='results-setup'>>=
...
plottype.accuracy <- modeldata.long %>% 
  ...
  group_by(plottype) %>%
  transform(total.acc = sum(accuracy))
plottype.accuracy$plottype <- with(plottype.accuracy, factor(plottype, levels=plottype[answer.type=='neither'][order(accuracy[answer.type=='neither'], accuracy[answer.type=="trend"])]))
...
<<participant-confidence,echo=F, fig.width=5, fig.height=3, out.width='.5\\linewidth', fig.cap="Participant confidence levels compared with trial results. \\label{fig:conflevel}">>=
ggplot(data=modeldata, 
       aes(x=factor(conf_level), 
           group=factor(c("both", "neither", "cluster", "cluster", "trend", "trend", "neither")[as.numeric(factor(outcome))], 
                        levels=c("both", "cluster", "trend", "neither")), 
           color=factor(c("both", "neither", "cluster", "cluster", "trend", "trend", "neither")[as.numeric(factor(outcome))], 
                        levels=c("both", "cluster", "trend", "neither")))) + 
...
<<speedaccuracy,echo=F, fig.width=8, fig.height=4, out.width='.8\\linewidth', fig.cap="Accuracy (Identifying either target plot) compared with mean trial time.", dependson='results-setup'>>=

time.accuracy <- modeldata %>% 
  group_by(individualID) %>%
  transform(trial.num=order(start_time)) %>%
  filter(trial.num>1)  %>%
  group_by(plottype, dataset) %>% 
  summarise(mean.trial.time = mean(trial.time), accuracy = 1-mean(neither.correct)) %>% 
  arrange(mean.trial.time) %>% 
  melt(id.var=c("plottype", "dataset", "mean.trial.time")) 
...
mean.time <- time.accuracy %>% group_by(plottype) %>% summarise(trial.time=sprintf("Avg. Time:\n%.1f s", as.numeric(mean(mean.trial.time))))


qplot(data=time.accuracy, x=as.numeric(mean.trial.time), y=value, color=plottype) + 
  geom_smooth(span=1, aes(ymax=pmin(1, ..ymax..))) + 
  xlim(c(20,60)) + 
  xlab("Mean Time to Trial Completion by Plot\n(excluding each participant's 1st trial)") + 
  ylab("Accuracy") + 
  scale_color_discrete("Plot Aesthetic") + 
  geom_text(data=mean.time, aes(x=40, y=0, label=trial.time), inherit.aes=F, vjust=0, show_guide=F) + 
