...
$\sigma_C$ & Scatter around cluster centers &  .20, .25, 0.30, .35\\\hline
\end{tabular}
\end{center}
...
nulldist<- function(N, sT=0.3, sC=0.3) {
  nulls <- data.frame(t(replicate(N, {
    lp <- data.frame(t(replicate(18, {
      mix = mixture.sim(lambda=0.5, K=3, N=45, q=sC, sd=sT)
      reg <- lm(y~x, data=mix)
  ...
  trends <- replicate(10, {
    mix = mixture.sim(lambda=0, K=3, N=45, q=sC, sd=sT)
    reg <- lm(y~x, data=mix)
    c(fline=summary(reg)$r.squared)
  })
  
  clusters <- replicate(10, {
    mix = mixture.sim(lambda=1, K=3, N=45, q=sC, sd=sT)
...
<<simulationparameters,echo=F,include=T, fig.width=10, fig.height=10, out.width='.8\\linewidth'>>=
