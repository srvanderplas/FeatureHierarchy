...
during which time \Sexpr{length(unique(modeldata$individualID))} individuals completed \Sexpr{nrow(modeldata)} unique lineup evaluations. Participants who completed fewer than 10 lineups were removed from the study (\Sexpr{length(incomplete.participants)} participants, \Sexpr{length(incomplete.participant.data)} evaluations), and lineup evaluations in excess of 10 were also removed from the study (\Sexpr{length(extra.participant.data)} evaluations). 

\newtext{
Of the participants who completed at least 10 lineup evaluations, \Sexpr{round(mean(users$gender=="Male")*100)}\% were male, and 
\Sexpr{
round(mean(users$age%in%c("18-25", "26-30")))
}
\% were between 18 and 30 years of age. 
}
\newtext{
Participants were fairly well-educated: \Sexpr{round(mean(users$education%in%levels(users$education)[3:4])*100)}\% had at least some undergraduate education, and \Sexpr{round(mean(users$education%in%levels(users$education)[5:6])*100)}\% had at least some graduate education. 
}
