...
\begin{algorithm}\hfill\newline
  ...
  Output: $N$ points, in form of vectors $x$ and $y$. \comment{specify output}
  \begin{enumerate}
...
\begin{figure}[ht]
<<cluster, fig.width=8, fig.height=5, out.width='\\textwidth', echo=FALSE>>=
...
The generative model for null data  is created as a mixture model $M_0$ that draws $n_c \sim B_{N, \lambda}$ observations from the cluster model, and $n_T = N - n_c$ from the regression model $M_T$. Observations are assigned groups using k-means clustering, where the clustering is repeated until all $K$ clusters contain at least $N/(2K+1)$ points; this condition ensures that clusters will not attract participant attention for being too ``uneven" and thus bias the results. 
% 
