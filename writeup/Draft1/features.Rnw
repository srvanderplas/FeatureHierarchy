...
\subsection{Numerical Signal Strength}
...
time.model <- lmer(log(as.numeric(trial.time))~ simple + simple:plottype + first.trial  + (1|individualID) + (1|data_name), data=modeldata)
