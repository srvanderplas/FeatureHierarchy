...
\comment{How do the shapes we picked fare in the discussion of shapes by \citet{robinson:03} see http://stat-computing.org/newsletter/issues/scgn-14-1.pdf? }
...
As a measure of clustering we use a similar coefficient: here, we assess the amount of variability taken care of by including the grouping variable, compared to total variability. Note that for the purpose of clustering, variability is measured as the variability in both $x$ and $y$ from a common mean, i.e. we assume that the values in $x$ and $y$ are on the same scale (which we can easily achieve by an appropriate re-scaling).  
\comment{Formula?}
