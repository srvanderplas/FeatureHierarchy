...
modeldata$outcome <- paste(c("", "line")[1+as.numeric(modeldata$line.correct==1)], 
                           c("", "group")[1+as.numeric(modeldata$group.correct==1)], 
...
modeldata <- ddply(modeldata, .(k), transform, line.diff=c("easy", "medium", "hard")[as.numeric(droplevels(sd.line))], cluster.diff=c("easy", "medium", "hard")[as.numeric(droplevels(sd.cluster))])

parameter.design <- unique(modeldata[,c(29, 17, 32, 33)])
@
...
<<participant-confidence,echo=F, fig.width=5, fig.height=4, out.width='.5\\linewidth', fig.cap="Participant confidence levels compared with trial results. \\label{fig:conflevel}">>=
...
<<speedaccuracy,echo=F, fig.width=8, fig.height=4, out.width='.8\\linewidth', fig.cap="Accuracy (Identifying either target plot) compared with mean trial time.">>=
...
qplot(data=time.accuracy, x=as.numeric(mean.trial.time), y=value, color=plottype) + geom_smooth(span=1, aes(ymax=pmin(1, ..ymax..))) + xlim(c(20,60)) + xlab("Mean Trial Time (by Plot)") + ylab("Accuracy") + facet_wrap(~plottype, nrow=2) + scale_color_discrete("Plot Aesthetic") + theme_bw()
@
...
line.model <- glmer(line.correct ~ 
                      # line.diff*cluster.diff*k + 
                      plottype + 
                      (1|individualID) + 
                      (1|dataset), 
                    data = modeldata, 
...
line.ranef$line.diff <- factor(line.ranef$line.diff, levels=c("easy", "medium", "hard"))
line.ranef$cluster.diff <- factor(line.ranef$cluster.diff, levels=c("easy", "medium", "hard"))
line.ranef$diff <- sprintf("Line: %s, Cluster: %s", line.ranef$line.diff, line.ranef$cluster.diff)
...
print(xtable(group.fixef2[,c(8, 1:4)], caption=c("Fitted values of fixed effects for the model described in \\eqref{eqn:groupModel}.", "Fixed effects for group target logistic model"), label="tab:group.fixef", align=c('r', 'r', 'r', 'r', 'r', 'r'), digits=c(0, 0, 4, 4, 2, 4)), include.rownames=F, file="figure/group-fixef-table")
@
