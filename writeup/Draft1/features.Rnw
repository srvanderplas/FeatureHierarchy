...
We begin by generating cluster centers along a line, then we generate points around the cluster center. \\
  ...
  \begin{enumerate}
    ...
    \item Generate points around cluster centers: 
      \begin{enumerate}
        \item $x^\ast_i = c^x_{g_i} + e$, $e_i \sim N(0, \sigma_C)$
        \item $y^\ast_i = c^y_{g_i} + e$, $e_i \sim N(0, \sigma_C)$
      \end{enumerate}
  \end{enumerate}
...
This model has the parameter $\sigma_T$ to reflect the amount of scatter around the trend line. 
  ...
  \begin{enumerate}
    \item Generate $x_i$, $i=1, ..., N$, a sequence of evenly spaced points from $[-1-2\sigma_T, 1+2\sigma_T]$ ($\sigma_T$ added and subtracted to match the range of cluster points in $x$)
    \item Jitter $x_i$: $x_i = x_i + \eta_i$, $\eta_i \sim Unif(-z, z)$, $z = 1/5*(2+4\sigma_T/(N-1)$
    \item Generate $y_i$: $y_i = a * x_i + e_i$, $e_i \sim N(0, \sigma_T)$
  \end{enumerate}
