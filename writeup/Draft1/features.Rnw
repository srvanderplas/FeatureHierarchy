...
\comment{For $K=3$ we have one shape each from Robinson's group 1, 2, and 9 - which all show differences in their preattentive properties; for $K=5$, we have one shape each from groups 1, 2, 3, and 9, 10. }
...
\begin{algorithm}\hfill\newline
  ...
  \begin{enumerate}
    \item Generate $x_i$, $i=1, ..., N$, as a sequence of evenly spaced points from $[-1, 1]$ ($\sigma_T$ added and subtracted to match the range of cluster points in $x$) \comment{I don't understand the text in the parenthesis}
    \item Jitter $x_i$ by adding small uniformly distributed perturbations to each of the values: $x_i = x_i + \eta_i$, $\eta_i \sim \text{Unif}(-z, z)$, $z = 1/5*(2/(N-1)$
    \comment{There's something going on with the parentheses: two opening, only one closing; we also cannot write $x_i = x_i + \eta_i$ -- that's programming, not math. so rename any of the pre-jittered $x$s to $\tilde{x}$. }
...
@
\caption{\label{fig:trends} Set of scatterplots showing one draw each from the trend model $M_T$ for $\sigma_T = 0.2, 0.3, 0.4, 0.5$.}
\end{figure}
