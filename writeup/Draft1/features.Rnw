...
<<null-distribution, echo=FALSE, include=FALSE>>=
...
<<samplepics, echo=F, fig.width=6, fig.height=6, out.width=".45\\linewidth">>=
source("../../Code/MixtureLineups.R")
...
d_ply(data, .(set), function(df){
  ...
  for(j in 1:nrow(plot.parms)){
    ...
    ggsave(plot = p, 
           filename = sprintf("figure/set-%d-plot-%d.png", i, j), 
