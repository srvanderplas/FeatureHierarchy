...
%       \end{enumerate}
    ...
    \newcomment{Let's not worry about getting $q$ from a random distribution, but let's rename it somehow, so it reflects the within cluster deviation a bit better ... $\sigma_C$?}
  \end{enumerate}
% \begin{enumerate}
%   \item Generate cluster centers along a line, then generate points around the cluster center. \\
%   Algorithm: \\
