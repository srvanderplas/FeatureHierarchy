...
\item[$\alpha$, $\beta$] are vectors of fixed effects, where $\beta$ is a vector corresponding to the nine designs investigated, and $\alpha$ encompasses all `other' fixed effects that might have an effect on the dependent variable besides the design of a plot,
