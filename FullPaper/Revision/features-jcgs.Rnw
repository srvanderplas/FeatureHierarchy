...
\end{itemize}
\hfill (pdf file)
\item[Supplement.zip:] \newdo{XXX we need to spill each file out by name - could you include the zip file on the repo? XXX } Zip directory containing files to recreate the figures and analysis presented in the paper and appendix. A README file is included with explanations about file content.
\end{description}
