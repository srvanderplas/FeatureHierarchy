...
\begin{figure}[ht]
<<trends, fig.width=6.5, fig.height=1.7, out.width='.8\\textwidth', echo=FALSE>>=
...
\begin{figure}[bht]
<<cluster, fig.width=5.2, fig.height=2.8, echo=FALSE>>=
...
\begin{figure}[hbt]
<<lambda, fig.width=6.5, fig.height=3.25, out.width='\\textwidth', echo=FALSE>>=
...
\begin{figure}[ht]
\centering
<<null-distribution-1, echo=FALSE, cache=T, fig.width=6.5, fig.height=2.2, out.width = "\\textwidth">>=
source("../../Code/MixtureLineups.R")
sT = 0.25
sC = 0.20
N = 45
...
<<demographics-1, echo=FALSE, fig.width=3.575, fig.height=1.7, fig.keep='all', fig.show='hold', out.width='\\textwidth'>>=
tmp <- users %>%
  group_by(age) %>%
  summarize(count = n()) %>%
  mutate(
...
ggplot(data = tmp) + 
  ...
  theme(axis.text.x = element_blank(), axis.title.x = element_blank(), plot.title = element_text(size = 12)) + 
  ggtitle("Participant Age Distribution")
@
\end{subfigure}
\begin{subfigure}[t]{0.43\textwidth}
<<demographics-2, echo=FALSE, fig.width=2.795, fig.height=1.694, fig.keep='all', fig.show='hold', out.width='\\textwidth'>>=
tmp <- users %>%
  group_by(education) %>%
  summarize(count = n()) %>%
  mutate(
...
\begin{figure}[ht]
\centering
<<targets, fig.width=5.2, fig.height=2.25, out.width='0.8\\textwidth', echo=FALSE>>=
...
ggplot() +
  ...
  theme_bw() +
  theme(axis.text.x = element_text(vjust=1), legend.position = "bottom") +
