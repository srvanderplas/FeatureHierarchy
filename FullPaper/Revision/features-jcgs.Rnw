...
\end{align}
where \begin{itemize}
\item[$\alpha$] is a vector of fixed effects, $\alpha_T$ and $\alpha_C$ for the effect of the standard error around trend lines $s_T$ and clusters $s_C$, and $\alpha_K$ for the effect of the number of clusters $K$,
...
\newdo{XXX include discussion of the effects for $s_C$ and $s_T$ -- they are exactly the way that we would hope: as errors increase, the probability for detecting the corresponding type of plot goes down significantly.}
...
\newdo{refer to table, explain plots}
\newdo{Expand table for estimates of $\alpha$}
