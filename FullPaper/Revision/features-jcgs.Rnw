...
\newtext{Performance on lineups has been shown to depend primarily on logical reasoning ability, and does not depend significantly on statistical training \citep{visualreasoning}.}
...
\newtext{This is analogous to a Bayesian framework in which the relative strengths of two competing models are compared with each other and with a common null.}
\newtext{The search for multiple targets is a more demanding cognitive task \citep{cain2011anticipatory} that is more sensitive to contextual effects \citep{adamo2015targets}, but without any time constraints imposed by the experimental protocol, participants can be expected to identify at least one of the plots with accuracy comparable to a single-target search task. Identification of the less-salient target can be expected to suffer due to ``satisfaction of search",\citep{fleck2010generalized} but this will emphasize the any differences between the two competing targets.}
...
\newdo{And this can go in the text again!!!}
...
modeldata <- merge(modeldata, lps[,c("set","k_new","sdline_new","sdgroup_new")], by.x="dataset", by.y="set")
...
\end{align}
where \begin{itemize}
\item[$\alpha$] \newtext{is a vector of fixed effects, $\alpha_T$ and $\alpha_C$ for the effect of the standard error around trend lines $s_T \in \{0.25, 0.35, 0.45\}$ and clusters $s_C \in \{0.2, 0.25, 0.3, 0.35\}$, and $\alpha_K$ for the effect of the number of clusters $K \in \{3, 5\}$,}
...
\newtext{The estimates for parameters $\alpha_C$ and $\alpha_T$, quantifying the effect of increased variability within clusters and around the trend line, are strongly significant (see Table~\ref{tab:results} for exact numbers) and work as hypothesized: as the variability increases, the strength of the target's signal decreases and correspondingly the probability for detecting the corresponding target decreases significantly.}
...
Numerically, these null data sets had uneven cluster allocation; bounding ellipse estimation failed for groups with fewer than 3 points and in these cases, ellipses were not drawn. Visually, the conspicuous absence of an ellipse led participants to select null plots with that feature. Appendix \ref{sec:gini} provides more information about the unintentional differences in group sizes between null and cluster target plots. 
}
...
$\alpha_C$ & 16.254 & 13.276 & 19.231 \\ 
$\alpha_T$ & -16.038 & -21.935 & -10.140 \\ 
...
% \newdo{Susan, I'm assuming there is some cognitive load thing going on? If so, could you summarize that impact?  Thanks!}
