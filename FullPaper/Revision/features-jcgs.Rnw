...
We further see a significant effect of Gini impurity on accuracy for  individual plot types ($\chi^2_9$=\Sexpr{round(gols.aov.2b$Chisq[2],1)}, $P$-value $< 0.0001$).
\newtext{This implies that homogeneity (or rather missing homogeneity) has a stronger effect in  some plot types than others. Figure~\ref{fig:two-way} shows the relationship between predicted accuracy and plot types for levels of homogeneity between 0.5 and 1 (similar to the range of Gini impurity observed in the study). Plots making use of color are all affected more strongly by deviations from homogeneity than plots without color. If ellipses are drawn, this effect becomes even more pronounced.
...
\end{itemize}
%\newdo{What does this even mean? As gini impurity increases, the probability of selecting the cluster target decreases significantly more when the color aesthetic is included, compared to the shape aesthetic...? (I'm making stuff up here, but I honestly can't parse this sentance without running the models outside of knitr... ugh, which means the reviewers won't be able to either.)

%HH: OK, we can explain this a bit ore: homogeneity (or missing homogeneity) is more obvious in some plot types than others, and yes, color is affected by homogeneity, as are the ellipses. Should we add a plot similar to the last one? try to come up with a plot, and we can decide then. XXX See figure~\ref{fig:two-way}, sorry about the colors, I tried to make up for them by adding the shapes. }
