...
%\tableofcontents
\newpage
\newdo{HH: for consistency: how do we call the different plot types officially? It shouldn't be plot type, because then I'd be thinking of scatterplots, boxplots and histograms rather than different aesthetics. But aesthetics doesn't capture things exactly either, because of the additional information in ellipses and trend lines. How about `designs' ? 
Once we agree on a term, we should go through the paper and fix it ...
}
...
\begin{description}
\item[Accuracy (in section~\ref{sec:accuracy}):] a lineup is considered to be evaluated `accurately', if at least one of its two targets is identified in an evaluation. We model this probability, i.e.\ $P(C_{ijk} \cup T_{ijk})$, where $C_{ijk}$ and $T_{ijk}$ are  binary variables describing the events that participant $j$ identifies the cluster/trend target in dataset $k$ presented with design $i$.
...
Plot type & $\beta$ & \checkmark & \checkmark & \checkmark & \checkmark  \\
...
{\bf Plot type $\beta$} \hfill Plain & 0.00 & --- & --- & --- \\ 
...
\newtext{In other words, a small Gini index indicates the presence of a null plot with large differences in the number of elements in wach group. This served as a(n unintended)  distractor away from the intended targets and therefore led to a drop in accuracy. Different plot types are affected differently strong (two-way interaction: $\chi^2_9$=\Sexpr{round(gols.aov.2b$Chisq[2],1)}, $P$-value $< 0.0001$): Plots making use of color are all affected more strongly by deviations from homogeneity than plots without color. If ellipses are drawn, the effect becomes even more pronounced. Figure~\ref{fig:two-way} gives an overview of the relationship between predicted accuracy and plot types for levels of homogeneity between 0.5 and 1 (similar to the range of Gini impurity observed in the study).}
...
{\bf Plot Type $\beta$} \hfill  Plain & 0.000 & --- & --- & --- \\ 
...
{\bf Plot type $\beta$} \hfill Shape & -0.006 & 0.034 & -0.187 & 0.8518 \\ 
  Plain  & 0.000 & --- & --- & ---\\
...
{\bf Plot Type $\beta$} \hfill  Trend + Error & -0.650 & -0.877 & -0.423 \\ 
...
We also find a significant effect if we additionally take the two-way interaction between a single missing ellipse and individual plot types into account ($\chi_9^2$=\Sexpr{round(aov.7b$Chisq[2],1)}, $P$-value=\Sexpr{round(aov.7b$`Pr(>Chisq)`[2],4)}). These effects are summarised in Figure~\ref{fig:gvl7b}.
\begin{figure}
