...
gol <- glmer(
  one.correct~sdline_new+sdgroup_new+k_new + plottype + (1 | data_name) + (1 | individualID), 
...
#  data = modeldata, family = binomial(), control = glmerControl(optimizer = "bobyqa")
#)
gol.fixef <- data.frame(confint(gol, method="Wald"))[-(c(1:2,4:6)),] # exclude sigmas
names(gol.fixef) <- c("LB", "UB")
gol.fixef$OR <- fixef(gol)[-(2:4)]
gol.fixef[1,1:3] <- 0
...
gol.fixef$label <- gsub("plottype", "", names(fixef(gol)))[-(2:4)]
...
\text{logit }P(C_{ijk} \cup T_{ijk}) = X\alpha ...
\end{equation}
