...
% Sat Jan  9 23:22:34 2016
\begin{table}[ht]
\centering
\begin{tabular}{lrrr}
  \hline
plot type & Odds Ratio & LB & UB \\ 
  \hline
Trend + Error & -0.6512 & -0.8774 & -0.4250 \\ 
  Color + Ellipse + Trend + Error & -0.5143 & -0.7497 & -0.2789 \\ 
  Plain & 0.0000 & --- & --- \\ 
  Trend & 0.1292 & -0.1013 & 0.3598 \\ 
  Color & 0.2709 & 0.0329 & 0.5088 \\ 
  Shape & 0.2764 & 0.0437 & 0.5092 \\ 
  Color + Shape & 0.3164 & 0.0794 & 0.5534 \\ 
  Color + Trend & 0.4585 & 0.2183 & 0.6987 \\ 
  Color + Ellipse & 0.4605 & 0.2087 & 0.7122 \\ 
  Color + Shape + Ellipse & 0.5722 & 0.3160 & 0.8284 \\ 
   \hline
\end{tabular}
...
modeldata$pv <- with(modeldata, paste("s_T=", sd.line,", s_C=",sd.cluster, sep=""))
prodplot(data=subset(modeldata, k==3), ~simpleoutcome+label+pv) + aes(fill=simpleoutcome) +
  scale_fill_manual("Outcome", values=c("darkred", "grey40", "steelblue", "grey90")) + ylab("") + theme_bw() + 
  facet_grid(pv~., scales="free", space="free") +
...
\end{figure}
\begin{figure}
<<complex-1b,echo=FALSE, fig.width=8, fig.height=10, out.width='\\textwidth'>>=
modeldata$set <- as.numeric(gsub("set-([0-9]+)-.*", "\\1", as.character(modeldata$data_name)))
load("../../Data/Lineups.Rdata") # includes object data.stats
data.stats <- data.stats %>% group_by(sd.trend, sd.cluster) %>% mutate(
  sd.trend.new = mean(sd.trend.est),
  sd.cluster.new = mean(sd.cluster.est)
)


modeldata <- merge(modeldata, data.stats[,c("set", "sd.trend.new", "sd.cluster.new")], by="set", all=T)
modeldata$pv_new <- with(modeldata, paste("s_C=", round(sd.cluster.new,3),", s_T=",round(sd.trend.new,3), sep=""))

ggplot(data=subset(modeldata, k==3)) +
  ...
  theme(axis.text.y=element_blank(), axis.ticks.y=element_blank(), 
        ...
        legend.position="bottom", strip.text.y = element_text(angle=0))
@
\caption{For $k=3$, estimated values. }
\end{figure}
\begin{figure}
<<complex-2,echo=FALSE, fig.width=8, fig.height=10, out.width='\\textwidth'>>=
library(productplots)
modeldata$simpleoutcome <- factor(modeldata$simpleoutcome, levels=c("trend", "both", "cluster", "neither"))
modeldata$label <- factor(modeldata$label, levels=gvl.fixef$label[order(gvl.fixef$OR)])
modeldata$pv <- with(modeldata, paste("s_T=", sd.line,", s_C=",sd.cluster, sep=""))
prodplot(data=subset(modeldata, k==5), ~simpleoutcome+label+pv) + aes(fill=simpleoutcome) +
  scale_fill_manual("Outcome", values=c("darkred", "grey40", "steelblue", "grey90")) + ylab("") + theme_bw() + 
  facet_grid(pv~., scales="free", space="free") +
  theme(axis.text.y=element_blank(), axis.ticks.y=element_blank(), 
        axis.title=element_blank(), 
