...
\newtext{These features are  discussed in more detail in the accuracy model in appendix section~\ref{sec:accuracy}. To investigate their effect on the balance between target models they are also investigated in the faceoff models of  appendix section~\ref{sec:faceoff}.}
