...
gol.2 <- update(gol.model, .~.+gini.min) # large gini (i.e. homogeneity) leads to significant better probability of picking one of the targets
gol.2b <- update(gol.2, .~.+gini.min:plottype) 
gol.3 <- update(gol.model, .~.+one.ellipse) # not significant, nor is the number of missing ellipses, nor the absence of at least one ellipse
gol.3b <- update(gol.3, .~.+one.ellipse:plottype) 
gol.4 <- update(gol.model, .~.+max.range) # not significant
gol.4b <- update(gol.model, .~.+max.range:plottype) # not significant

gol.5 <- update(gol.model, .~.+log(time)) # there is a (highly) significant effect of (log) response time on accuracy. The size of the effect is on average -0.405486 for each unit increase in (log) response time.
...
\subsection{Effect of missing ellipses}\label{sec:gini}
...
\newtext{The probability to pick at least one of the targets significantly increases with an increase in gini impurity, indicating a higher amount of homogeneity in group sizes ($\chi^2_1 = 3.94$, $P$-value = 0.0472). We further see a significant effect of gini impurity on individual plot types ($\chi^2_9 = 34.69$, $P$-value = 0.0001). }
...
\newtext{These results suggest that the speed of evaluation is not significantly contributing on the balance between selecting one target over the other. 
}
