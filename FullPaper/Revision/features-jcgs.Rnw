...
Supplementary materials for this paper are available online: a supplemental PDF provides additional details about the methodology and model, and code and data are available on \href{http://github.com/srvanderplas/FeatureHierarchy}{github}.
\end{abstract}
...
% competing signals
The two targets represent two different, competing signals; an observer's choice then demonstrates empirically which signal is more salient. The probability distribution which underlies the hypothesis testing framework changes slightly with the addition of a second target; Supplement~\ref{app:probabilitysimulation} contains more details. Single-target lineups rely on the hypothesis testing framework; the dual-target lineup is more simlar to a Bayesian comparison of the relative strengths of two competing models with respect to a common null. 
...
\newtext{
We modeled accuracy rates, response times, participant confidence, and the selection of cluster vs. trend targets in great detail. A thorough discussion of these models and the corresponding results is provided in Supplement~\ref{app:models}; only the models describing target plot selection are discussed here due to space constraints.
}
...
Figure~\ref{fig:onetarget} shows an overview of the number of evaluations by design (outlines) and the number of times participants chose at least one of the targets (dark shaded areas). Additional information about the model structure, parameter estimates, and interpretations are provided in Supplement~\ref{app:accuracy}. 
...
\newtext{The addition of an interaction effect between plot design and levels of $\alpha_C$ or $\alpha_T$ did not significantly improve the predictive ability of the model. That is, the selection of the cluster or trend target does not appear to be influenced by the combination of a plot design and the difficulty of the parameters controlling data generation (within the ranges of parameters examined in this experiment) outside of the individual effects of plot design and trend/cluster variability.}
...
\begin{itemize}
\item Supplement \ref{app:parametersimulation}: Details about simluations of the data-generating model described in section~\ref{sec:parameterselection}.
\item Supplement \ref{app:probabilitysimulation}: Probability distribution for hypothesis testing in dual-target lineups.
\item Supplement \ref{app:models}: Contains a thorough review of models fit to participant target selections, response time, and self-reported confidence. Additional modifications of these models address participant reasoning as discussed in section~\ref{sec:sentiment}, the relationship between participant response time and accuracy, and other interesting features of the data.
\end{itemize}
