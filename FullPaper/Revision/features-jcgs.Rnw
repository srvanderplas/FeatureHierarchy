...
Cognitively, the presence of two targets leads to a dual-target search scenario~\citep{fleck2010generalized}, which effectively introduces a `masking' effect where the mor salient target is selected and the search for more targets stops (``satisfaction of search"), i.e.\ people tend to pick the more salient target and not notice the second target, even though in the absence of the more salient target they would have picked it out. 
From a statistical perspective, 
% like Bayes
this is analogous to a Bayesian framework in which the relative strengths of two competing models are evaluated by a comparison with each other and with a common null.
...
%If both targets exhibit similar signal, observers may identify both targets, 
% search for two
...
\begin{figure}[hbt]
<<lambda, fig.width=8, fig.height=3.5, out.width='.9\\textwidth', echo=FALSE>>=
...
& Strength & 0 & 1 & 2 \\
\cellcolor{gray!25}\begin{tabular}{c} \phantom{.}\\ \phantom{.} \end{tabular} & 0 &  \cellcolor{gray!5} None &  \cellcolor{gray!15} Trend &  \cellcolor{gray!25} Trend + Prediction \\
...
\end{subfigure}
\begin{subfigure}[t]{0.24\textwidth}
  \caption{Trend + Prediction }\vspace{-0.15in}
...
\centering
  \caption{\scalebox{.8}{Trend + Prediction + Color + Ellipse}}\vspace{-0.15in}
...
\newdo{What did participants have to do: (i) read through legal matters, (ii) pass two trials of easy (single target) lineups, (iii) give feedback on a set of lineups, (iv) respond to demographics at some point (voluntary).
}
...
gol.5b <- update(gol.2b, .~. + poly(-log(time),2), 
                control = glmerControl(
                  optimizer="bobyqa",
                  optCtrl = list(maxfun = 1e5)
                ))
...
Plot types  associated with  clustering as shown in Table~\ref{tab:plotaesthetics} lead to significantly fewer correct evaluations ($\chi^2_{9} = 387.9$, $p$-value $< 0.0001$). A  possible cause for this is discussed in more detail in section~\ref{sec:sentiment}. 
...
# qplot(reorder(label,label,function(x) -length(x)), data=faceoff) + coord_flip() + theme_bw() + xlab("") + ylab("# of target evaluations") + geom_bar(aes(x=label), data=modeldata, fill=NA, colour="black")
...
# ggplot() + 
#   theme_bw() + 
#   xlab("# of target evaluations") + ylab("") + 
#   facet_grid(tendency~., space="free_y", scales="free_y") +
#   geom_rect(aes(xmin=0, xmax=eval, ymin=nlabel-.4, ymax=nlabel+0.4, fill=type), 
#            data=subset(totals, type=="Total"), stat="identity", 
#            position="identity", colour="black") + 
#   geom_rect(aes(xmin=0, xmax=eval, ymin=nlabel-.4, ymax=nlabel+0.4, fill=type), 
#            data=subset(totals, type=="Correct"), stat="identity", 
#            position="identity", colour="black") + 
#   scale_y_continuous(breaks=1:10, labels=md$label) +
...
As hypothesized, the strongest signal for identifying groups, is color + shape + ellipse, while trend + prediction results in the strongest signal in favor of trends. 
...
The estimates for parameters $\alpha_C$ and $\alpha_T$, quantifying the effect of increased variability within clusters and around the trend line, are highly significant (see Table~\ref{tab:results} for exact numbers) and work as hypothesized: with an increase in the variability, the strength of the target's signal decreases and correspondingly the probability for detecting the corresponding target decreases significantly.

\begin{figure}[h!bt]\centering
<<group-vs-line, echo=FALSE, fig.width=9, fig.height=4, out.width='.95\\textwidth', warning = FALSE>>=
...
When trend line and prediction intervals (``error bands") are present in the same plot, the additional Gestalt principle of common region is recruited, in addition to the continuity heuristic present due to the trend line and the linear relationship between $x$ and $y$. The interaction between these heuristics dominates the perceptual experience, decreasing the probability that a participant will select the cluster target plot in favor of the trend target. 

This interaction effect explains the different outcomes seen by the two conditions with conflicting aesthetics: the color+trend condition is more likely to result in cluster plot selection, while the color + ellipse + trend + prediction condition is more likely to result in trend plot selection, because the combined effect of the gestalt heuristics present in the trend and prediction interval elements is stronger than the effect of color and ellipse elements, which only invoke Gestalt heuristics of similarity and common region. 
...
%' <<lexical-analysis, echo=FALSE, fig.width=8, fig.height=6, out.width='.8\\textwidth'>>=
%' lexicaldata <- modeldata
%' lexicaldata$choice_reason <- tolower(lexicaldata$choice_reason)
%' 
...
The estimation of bounding ellipses fails for groups with fewer than three points and in these cases, ellipses were not drawn. Visually, the conspicuous absence of an ellipse led participants to select null plots with that feature. \newtext{When we include a variable encoding the absence of one of the ellipses in a lineup into model~(\ref{eqn:faceoffModel}) it is highly significant ($\chi^2_1 =$\Sexpr{round(aov7$Chisq[2],4)}, $P$-value=\Sexpr{round(aov7$`Pr(>Chisq)`[2],4)}), and its estimate is -0.8075, indicating that if one of the ellipses in a lineup is missing, the probability of picking the cluster target is reduced to less than half (44.6\%) of the trend target. If we additionally consider the effect of a missing ellipse on individual plot types, it is also significant ($\chi^2_1 =$\Sexpr{round(aov7b$Chisq[2],4)}, $P$-value=\Sexpr{round(aov7b$`Pr(>Chisq)`[2],4)}).  Appendix section~\ref{sec:gini:2} gives more details on this model and its effects.}
%Appendix section~\ref{sec:gini} provides more information about the unintentional differences in group sizes between null and cluster target plots. 
...
\newtext{The lineup protocol allows us an easy way of measuring accuracy of  evaluations by assessing the number of participants who identified the data plot. In the modified version, we can use this as well by regarding any lineup evaluation resulting in an identification of at least one of the two targets as `accurate'. We therefore want to model the probability that}
\begin{align}
\text{logit }P(C_{ijk} \cup T_{ijk}) & = \textbf{W}\alpha +  \textbf{X}\beta + \textbf{J}\gamma + \textbf{K}\eta + \epsilon, \label{eqn:responseModel}
\end{align}
where \begin{itemize}
\item[$\alpha$] is a vector of fixed effects $(\mu, \alpha_T, \alpha_C, \alpha_K, \alpha_F)$, where $\mu$ an average baseline accuracy (and should not be interpreted, because $s_C$ and $s_T$ are assumed to be zero),  $\alpha_T$ and $\alpha_C$ for the effect of the standard error around trend lines $s_T \in \{0.25, 0.35, 0.45\}$ and clusters $s_C \in \{0.2, 0.25, 0.3, 0.35\}$, and $\alpha_K$ for the effect of the number of clusters $K \in \{3, 5\}$, $\alpha_O$ an order effect, i.e.\ an effect on accuracy over time,
\item[$\beta_i$] describe plot types,
\item[$\gamma_j$] $\overset{iid}{\sim} N\left(0, \sigma^2_{\text{data}}\right)$,  random effect for dataset specific characteristics,
\item[$\eta_k$] $\overset{iid}{\sim} N\left(0, \sigma^2_{\text{participant}}\right)$, random effect for participant characteristics,
\item[$\epsilon_{ijk}$] $\overset{iid}{\sim}  N\left(0, \sigma^2_e\right)$, error associated with a single trial evaluation.
\end{itemize}
We also assume that random effects are orthogonal.

<<gols, echo=FALSE>>=
gols.aov.2 <- anova(gol, gol.2)
gols.aov.2b <- anova(gol.2, gol.2b)
gols.aov.5 <- anova(gol.2b, gol.5)
...
%' @
\newtext{Table~\ref{tab:gol} shows an overview of the parameters of the accuracy model and their estimates. Both $\alpha_T$ and $\alpha_C$ have large negative effects that are highly significant. This indicates that as the signal in the target plots weakens (by an increase in variability around the trend line or within cluster variability), accuracy of participants decreases on average. $\alpha_K$ has a small negative effect, i.e.\ participants are on average answering lineups with three clusters with more accuracy than lineups with five clusters. However, this effect is not significant.
The order effect is small, but significant; as participants answer more lineups, their accuracy decreases on average by about 2\% for each evaluation. }
\newdo{XXX Susan, this is a fatigue effect -- do you have a reference for this?}

\begin{table}[ht]
% xtable(summary(gol)$coefficients, digits=c(4))
\centering
\begin{tabular}{rrrrr}
  \hline
Parameter & Estimate & Std.\ Error & $z$-value & Pr($>$$|$z$|$) \\ 
  \hline
$\mu$ & 6.15 & 0.84 & 7.28 & 0.00 \\ 
  $\alpha_T$ & -2.55 & 1.20 & -2.13 & 0.03 \\ 
  $\alpha_C$ & -8.59 & 2.37 & -3.62 & 0.00 \\ 
  $\alpha_{K=3}$ & 0.00 & --- & --- & --- \\ 
  $\alpha_{K=5}$ & -0.21 & 0.23 & -0.93 & 0.35 \\ 
  $\alpha_O$  & -0.02 & 0.01 & -2.33 & 0.02 \\ [3pt]
{\bf Plot type} \hfill Plain & 0.00 & --- & --- & --- \\ 
  Trend + Error & -0.08 & 0.14 & -0.57 & 0.57 \\ 
  Shape & -0.08 & 0.14 & -0.57 & 0.57 \\ 
  Trend & -0.35 & 0.13 & -2.61 & 0.01 \\ 
  Color + Shape & -0.51 & 0.13 & -3.95 & 0.00 \\ 
  Color & -0.60 & 0.13 & -4.64 & 0.00 \\ 
  Color + Trend & -0.70 & 0.13 & -5.49 & 0.00 \\ 
  Color + Ellipse + Trend + Error & -1.12 & 0.12 & -9.05 & 0.00 \\ 
  Color + Shape + Ellipse & -1.40 & 0.12 & -11.45 & 0.00 \\ 
  Color + Ellipse & -1.47 & 0.12 & -12.09 & 0.00 \\ 
   \hline
\end{tabular}
...
The effect of (log) response times on accuracy is highly significant ($\chi^2_1 = $\Sexpr{round(gols.aov.5$Chisq[2],4)}, $P$-value = \Sexpr{round(gols.aov.5$`Pr(>Chisq)`[2],4)}). With each unit increase in (log) response time the probability for a target identification is reduced on average by about 1/3. However, in the long run, a secondary effect takes place, and response time has a positive effect on accuracy again. Fitting an additional quadratic term in the model is also highly significant ($\chi^2_1 = $\Sexpr{round(gols.aov.5b$Chisq[2],4)}, $P$-value = \Sexpr{round(gols.aov.5b$`Pr(>Chisq)`[2],4)}), and leads to an overall minimum accuracy over time at a response time of about 150 seconds.
...
\newtext{Other features measuring the imbalance within a lineup that we consider besides (a) gini impurity are (b)  the difference between the maximum and the minimum number of elements in each of the groups of a lineup, and (c) the number of ellipses missing from the lineup.}

\newtext{The probability to pick at least one of the targets significantly increases with an increase in gini impurity, indicating a higher amount of homogeneity in group sizes ($\chi^2_1 = $\Sexpr{round(gols.aov.2$Chisq[2],4)}, $P$-value = \Sexpr{round(gols.aov.2$`Pr(>Chisq)`[2],4)}). We further see a significant effect of gini impurity on individual plot types ($\chi^2_9 = $\Sexpr{round(gols.aov.2b$Chisq[2],4)}, $P$-value < 0.0001). }
...
\newtext{The range of group sizes does not have a significant effect on the probability to pick at least one of the targets, even if different plot types are taken into account.}

\newtext{Similarly, a single absent ellipse does not lead to a significant change in the probability of detecting one of the target plots. Neither have the number of missing ellipses nor the absence of at least one ellipse a significant effect on this probability, not even when we consider the impact of individual plot types.}
\newdo{they are all in the range of model gol.3}

\newtext{These features are also discussed in the faceoff model of section~\label{sec:faceoff} to investigate their effect on the balance between target models.}

%\newpage
\subsection{Modelling response times}\label{sec:response}
While we do not have the same amount of control in an AMT study that we would have an a lab setting, we can measure the time between presenting a lineup and the time that a participant submits the results on it. A histogram of these times is given in Figure~\ref{fig:histogram}. Response times are extremely skew. In the model we therefore use the log of response times $T = \left(t_{ijk}\right)_{n \times 1}$:
<<response, echo=FALSE>>=
...
\newdo{I don't  agree with the conclusion below: the dependent variable is not accuracy, but self-reported confidence. For speed-accuracy we need to fit a different model -- model gol.5 does attempt this, and shows that we do indeed have a highly significant speed-accuracy relationship, however, as response times are longer, accuracy goes down. }
These results suggest that the speed-accuracy tradeoff was not evident in this experiment: participants who identified one target plot correctly were faster than those who did not identify either target plot correctly. This may be due to the pre-selection of participants who were able to successfully complete at least one lineup task.
...
\newdo{This is a fun result! It goes hand in hand with the signal strength -- ellipses emphasize the absence of individual groups. }
...
%' 
%' This alternative looks pretty messy
%' <<alternative, echo=FALSE>>=
%' library(dplyr)
%' stats <- modeldata %>% group_by(k_new, sdline_new, sdgroup_new, dataset, plottype) %>% summarize(
%'   n = n(),
%'   neither = sum(simpleoutcome=="neither")/n(),
%'   cluster = sum(simpleoutcome=="cluster")/n(),
%'   trend = sum(simpleoutcome=="trend")/n(),
%'   both = sum(simpleoutcome=="both")/n()
%' )
%' 
%' library(reshape2)
%' ms <- melt(stats, measure.var=c("neither", "cluster", "trend", "both"))
%' names(ms)[1:3] <- c("K", "sT", "sC")
%' 
%' qplot(plottype, value, data=ms, group=dataset, shape=variable, colour=variable) + 
%'   facet_grid(facets=K+sC~sT, labeller="label_both") + 
...
@
Response time (split into log(response time) and effect of first trial) does not have a significant effect on the decision between cluster and trend target ($\chi_2^2 =$\Sexpr{round(aov$Chisq[2],4)}, $P$-value = \Sexpr{round(aov$`Pr(>Chisq)`[2],4)}). 
Nor does the confidence level of participants ($\chi_2^2 =$\Sexpr{round(aov2$Chisq[2],4)}, $P$-value = \Sexpr{round(aov2$`Pr(>Chisq)`[2],4)}). 

\subsubsection{Effect of missing ellipse}\label{sec:gini:2}
\newdo{Results for faceoff: Only a single missing ellipse leads to a significant effect(in favor of the trend target over the cluster target).}
\newdo{Parameter overview of model gvl.7b -- how are individual plot types affected by a single missing ellipse in one of the panels of the lineup? That's a two-way interaction, and a teensy bit messy to explain, because the plot type effects are changed from gvl.4.}
