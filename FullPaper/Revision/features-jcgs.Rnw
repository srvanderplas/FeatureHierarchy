...
gvl.7 <-  update(gvl.4, .~.+plottype*one.ellipse, control=glmerControl(optimizer="bobyqa"))
...
The estimation of bounding ellipses fails for groups with fewer than three points and in these cases, ellipses were not drawn. Visually, the conspicuous absence of an ellipse led participants to select null plots with that feature. Appendix section~\ref{sec:gini} provides more information about the unintentional differences in group sizes between null and cluster target plots. 
