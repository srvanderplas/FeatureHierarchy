...
\newtext{The estimates for parameters $\alpha_C$ and $\alpha_T$, quantifying the effect of increased variability within clusters and around the trend line, are strongly significant (see Table~\ref{tab:results} for exact numbers) and work as hypothesized: as the variability increase, the strength of the target's signal decreases and correspondingly the probability for detecting the corresponding target decreases significantly.}
...
<<sentiment, eval=T, echo=FALSE, fig.width=7, fig.height=7, out.width='.3\\textwidth', fig.show='hold'>>=
...
\begin{tabular}{rrrrr}
  \hline
Parameter & Estimate & Std. Error & z.value & P values \\ 
  \hline
...
\newdo{XXX need the model formula here XXX}
...
\newtext{Table~\ref{tab:conf} gives an overview of the parameters and estimates of the model. The longer a participant needs to evaluate a lineup, the lower on average will be the value of confidence reported with it. Similarly, an increase in lineup difficulty (as controlled by increased values of $s_C$ and $s_T$) goes hand in hand with a significant decrease in confidence. If neither one or both of the two targets were identified, the reported confidence level as significantly lower than if one of the two targets were identified. Aesthetics in general did not have a significant effect on confidence levels. However, individual aesthetics did lead to a significant increase in confidence: any plot showing ellipses increases the level of confidence on average by about 0.1. }
...
(Intercept) & 5.800 & 0.186 & 31.190 & $\le 0.0001$ \\ 
  ...
  plottypetrend & 0.049 & 0.034 & 1.434 & 0.1514 \\ 
  plottypecolor & 0.038 & 0.034 & 1.119 & 0.2630 \\ 
  plottypeshape & -0.010 & 0.034 & -0.280 & 0.7793 \\ 
  plottypecolorShape & 0.018 & 0.034 & 0.521 & 0.6023 \\ 
  plottypecolorEllipse & 0.063 & 0.034 & 1.834 & 0.0666 \\ 
  plottypecolorTrend & 0.049 & 0.034 & 1.449 & 0.1474 \\ 
  plottypetrendError & 0.049 & 0.034 & 1.429 & 0.1530 \\ 
  plottypecolorShapeEllipse & 0.101 & 0.034 & 2.935 & 0.0033 \\ 
  plottypecolorEllipseTrendError & 0.101 & 0.034 & 2.959 & 0.0031 \\ [3pt]
  simpleoutcometrend & 0.000 & --- & --- & --- \\ 
  simpleoutcomecluster & -0.022 & 0.023 & -0.989 & 0.3225 \\ 
  simpleoutcomeneither & -0.228 & 0.028 & -8.187 & $\le 0.0001$ \\
  simpleoutcomeboth & -0.231 & 0.103 & -2.247 & 0.0246 \\ 
   \hline
\end{tabular}
