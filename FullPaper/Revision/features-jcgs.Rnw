...
What does have a significant effect on the balance between cluster target and trend target, is the absence of one of the ellipses in one of the panels of the lineup: a single missing ellipse cuts the probability that the cluster target is selected in less than half (44.6\%; $\chi_2^1 =$\Sexpr{round(aov.7$Chisq[2],4)}, $P$-value = \Sexpr{round(aov.7$`Pr(>Chisq)`[2],4)}). 
