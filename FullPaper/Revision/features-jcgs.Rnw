...
@
\caption{\label{fig:two-way}Predicted accuracy values for plot types given different values of the gini impurity measure. Values of gini between 0.59 and 1 were actually observed in the panels of the lineups. Accuracy of all plot types with color are lower than their non-colored counterparts. With a large deviation from equi-distributed groups, accuracy is affected disproportionally strong.}
\end{figure}
