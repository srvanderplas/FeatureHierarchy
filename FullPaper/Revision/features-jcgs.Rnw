...
\newtext{The addition of an interaction effect between plot design and levels of $\alpha_C$ or $\alpha_T$ did not significantly improve the predictive ability of the model. This suggests that the selection of the cluster or trend target is not influenced by the combination of a plot design and the difficulty of the parameters controlling data generation (at least within the ranges of parameters examined in this experiment).}
