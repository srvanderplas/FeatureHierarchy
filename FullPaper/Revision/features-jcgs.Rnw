...
\begin{description}
\item[Paper Appendix:] Contains additional details about the methodology and results which were not included in this paper due to space constraints. (pdf)
\begin{itemize}
...
\end{itemize}
\item[Supplement.zip:] Zip directory containing files to recreate the figures and analysis presented in the paper and appendix. A README file is included with explanations about file content.
\end{description}
