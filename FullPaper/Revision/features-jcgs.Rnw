...
\begin{figure}[h!bt]\centering
<<group-vs-line, echo=FALSE, fig.width=9, fig.height=4, out.width='.95\\textwidth'>>=
...
\begin{figure}
<<histogram, echo=FALSE, fig.width=5, fig.height=3, out.width='.6\\textwidth'>>=
library(ggplot2)
...
\end{align}
where \begin{itemize}
\item[$\alpha$] is a vector of fixed effects, $\mu$ the average response time, $\alpha_0$ the average effect of the first trial on the response time,  $\alpha_T$ and $\alpha_C$ for the effect of the standard error around trend lines $s_T \in \{0.25, 0.35, 0.45\}$ and clusters $s_C \in \{0.2, 0.25, 0.3, 0.35\}$, and $\alpha_K$ for the effect of the number of clusters $K \in \{3, 5\}$,
\item[$\beta_i$] describe plot types,
\item[$\gamma_j$] $\overset{iid}{\sim} N\left(0, \sigma^2_{\text{data}}\right)$,  random effect for dataset specific characteristics,
\item[$\eta_k$] $\overset{iid}{\sim} N\left(0, \sigma^2_{\text{participant}}\right)$, random effect for participant characteristics,
\item[$\epsilon_{ijk}$] $\overset{iid}{\sim}  N\left(0, \sigma^2_e\right)$, error associated with a single trial evaluation.
...
Parameter & Estimate & Std. Error & z.value & P values \\ 
  \hline
$\mu$ & 2.660 & 0.108 & 24.586 & 0.000e+00 \\ 
  $\alpha_0$ & 0.231 & 0.014 & 16.366 & 0.000e+00 \\
    $\alpha_{K = 3}$ & 0.000 & --- & --- & --- \\ 
    $\alpha_{K = 5}$ & 0.124 & 0.029 & 4.334 & 1.464e-05 \\ 
  $\alpha_T$ & 0.461 & 0.152 & 3.039 & 2.376e-03 \\ 
  $\alpha_C$ & 1.703 & 0.300 & 5.673 & 1.407e-08 \\ [3pt]
{\bf Plot Type} \hfill  Plain & 0.000 & --- & --- & --- \\ 
  Shape & 0.112 & 0.019 & 5.889 & 3.885e-09 \\ 
  Color & 0.133 & 0.019 & 7.012 & 2.348e-12 \\ 
  Trend & 0.148 & 0.019 & 7.815 & 5.551e-15 \\ 
  Trend + Error & 0.166 & 0.019 & 8.759 & 0.000e+00 \\ 
  Color + Ellipse & 0.205 & 0.019 & 10.735 & 0.000e+00 \\ 
  Color + Shape & 0.214 & 0.019 & 11.281 & 0.000e+00 \\ 
  Color + Trend & 0.215 & 0.019 & 11.345 & 0.000e+00 \\ 
  Color + Shape + Ellipse & 0.205 & 0.019 & 10.699 & 0.000e+00 \\ 
  Color + Ellipse + Trend + Error & 0.252 & 0.019 & 13.215 & 0.000e+00 \\ [3pt]
  \bf Target \\
  Trend & -0.182 & 0.016 & -11.665 & 0.000e+00 \\ 
  Cluster & -0.150 & 0.013 & -11.165 & 0.000e+00 \\ 
  Neither & 0.000 & --- & --- & --- \\ 
  Both & 0.166 & 0.059 & 2.820 & 4.797e-03 \\ 
   \hline
\end{tabular}
