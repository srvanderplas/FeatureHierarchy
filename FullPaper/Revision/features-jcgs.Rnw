...
gol.3 <- update(gol.model, .~.+no.ellipse) # not significant
gol.4 <- update(gol.model, .~.+group.min) # not significant
...
\newtext{Other features measuring the imbalance is the difference between the maximum and the minimum number of elements in each of the groups of a lineup, and the number of ellipses missing from the lineup.}

\newdo{first model: probability to pick at least one of the targets: increased by gini. missing ellipse is not significant, nor is the minimal number of group elements. }
