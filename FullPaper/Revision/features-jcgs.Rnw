...
\subsection{Face-Off: Trend versus Cluster}\label{app:faceoff}
...
aov.5c <- anova(gol.5b, gol.5c)
@

\newdo{OK, so the interaction effect between response time and parameters controlling for difficulty is hugely significant: $\chi^2_2$=\Sexpr{aov.5c$Chisq[2],1)}, $P$-value=\Sexpr{round(aov.5c$`Pr(>Chisq)`[2],4)}. Both effects are negative, i.e. as  difficulty increases, the effects of fatigue are even stronger. }
...
df5 <- data.frame(expand.grid(
  ...
  k_new=factor(c(3,5)),
  trial.num=5,
  trial.time = exp(seq(1,7.5, by=0.25))
))
...
qplot(trial.time, pred5b, facets=sdline_new~sdgroup_new, 
      data=subset(df5, k_new==3), geom="line", group=plottype) + 
  geom_line(aes(y=pred, colour=plottype), size=1) + theme_bw() + 
...
@
\caption{\label{fig:interaction}In black: predictions from model with  polynomial response time. In colour: additional interactions between response time and each of the difficulty settings. }
\end{figure}
...
df2b <- data.frame(expand.grid(
  ...
  k_new = 3,
  trial.num=5,
  gini.min=seq(0.5,1, by=0.1)
))

cols <- brewer.pal(9, "Set1")

df2b$pred <- predict(gol.2, newdata=df2b, re.form=~0, type="response")
qplot(gini.min, pred, colour=plottype, shape=plottype,  data=df2b, size=I(2.5)) + theme_bw() + 
  geom_line(aes(group=plottype)) + 
