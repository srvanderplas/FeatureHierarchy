...
\clearpage
\section{Model Discussion and Results} \label{app:models}
...
{\bf Design $\beta$} \hfill Plain & 0.00 & --- & --- & --- \\ 
  ...
  Color + Ellipse & -1.47 & 0.12 & -12.09 & $< 0.0001$ \\ 
   \hline
\end{tabular}
\caption{\label{tab:gol}Parameters and estimates of the accuracy model~\ref{app:accuracy}. }
\end{table}
...
{\bf Design $\beta$} \hfill Shape & -0.006 & 0.034 & -0.187 & 0.8518 \\ 
  ...
  Color + Shape + Ellipse & 0.107 & 0.034 & 3.156 & 0.0016 \\ 
   \hline
\end{tabular}
\caption{\label{tab:conf}Parameters and estimates for the model of participants' confidence~\ref{app:confidence}.  }
\end{table}
