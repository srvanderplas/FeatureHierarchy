...
\if0\blind
{
  \title{\bf Supplement to: Clusters beat Trend!? \\Testing feature hierarchy in statistical graphics}
...
\if1\blind
{
  ...
  \bigskip
  \begin{center}
    {\LARGE\bf Supplement to: Clusters beat Trend!? \\Testing feature hierarchy in statistical graphics}
\end{center}
  \medskip
} \fi
