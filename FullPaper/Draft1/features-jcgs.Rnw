...
\noindent%
{\it Keywords:}  Visual inference, Lineup protocol, Preattentive Features, \comment{XXX do you have any more? } 3 to 6 keywords, that do not appear in the title
\vfill
...
\begin{figure}[bht]
<<cluster, fig.width=8, fig.height=4.5, out.width='\\linewidth', echo=FALSE>>=
...
res <- res %>% group_by(sd.cluster) %>% do(data.frame({set.seed(325098573); sim.clusters(K=3, N=45, sd.cluster=as.numeric(.[1,1]))}))
...
res2 <- res2 %>% group_by(sd.cluster) %>% do({set.seed(325098573); sim.clusters(K=5, N=75, sd.cluster=as.numeric(.[1,1]))})
...
\begin{figure}[hbt]
<<lambda, fig.width=8, fig.height=3.5, out.width='.9\\linewidth', echo=FALSE>>=
...
The parameter values were chosen in an approach similar to that taken in \citet{niladri:2014}: for each  combination of $\sigma_T\in\{0.2, 0.25, ..., 0.5\}$, $\sigma_C\in\{0.1, 0.15, ..., 0.4\}$, and $K\in\{3,5\}$ we simulated 1000 lineup datasets. Then trend and cluster strength indices, $R^$ and $C^2$, were computed  for the simulated target plots, and compared to the most extreme value for each of the 18 null plots of the same lineup data.

The resulting distributions allow us to objectively assess the difficulty of detecting the target datasets computationally (without relying on human perception) within the full parameter space. E.g., a target plot with $R^2=0.95$ is very easy to identify when surrounded by null plots with $R^2=0.5$, while null plots with $R^2=0.9$ make the target plot more difficult to identify. 
...
qplot(data=subset(longres, Statistic != "Gini Impurity"), x=value, y=..density.., stat="density", color=dist, fill=dist, geom="area", alpha=I(.5), 
    #  main=expression(paste("Simulation Results " , group("(", list(K==3, sigma[T]==.25, sigma[C]==.2), ")"))), 
      ...
      ylab="Density", 
      position="identity") + 
  facet_grid(.~Statistic,  labeller=label_both) + # better not use scales="free" 
...
\centering
  ...
  \includegraphics[width=0.25\linewidth]{figure/fig-samplepics-10}
\end{subfigure}
...
\hfill
\begin{subfigure}[t]{0.475\textwidth}
<<shape-palette, dev='cairo_pdf', echo=FALSE, fig.width=5, fig.height=1, out.width='\\linewidth',message=F, warning=F>>=
shapes <- c(1,0,3,4,8,5,2,6,-0x25C1, -0x25B7)

qplot(x=1:10, y=0, shape=shapes, size=I(5)) + scale_shape_identity() + theme_lineup()
@
\caption[Shape palette used to maximize preattentive perception]{Shape palette. Due to varying point size between Unicode and non-Unicode characters, the last two shapes were removed from our investigation.\label{fig:shapes}}
\end{subfigure}
...
In order to produce experimental stimuli accessible to the approximately 4\% of the population with red-green color deficiency \citep{colorvision}, we removed the gray hue from the palette. This modification resulted in maximally different color combinations which also did not include red-green combinations, while also removing a color (gray) which is difficult to distinguish for those with color deficiency.  
...
users <- read.csv("../../Data/turk16_users_anon.csv", stringsAsFactors=F, header=F)
...
users$education <- factor(users$education, levels=0:5, labels=c("NA", "High School or less", "Some college", "Bachelor's degree", "Grad school or higher", "Grad school or higher"))
...
<<demographics-1, echo=FALSE, fig.width=8.53, fig.height=4, fig.keep='all', fig.show='hold', out.width='\\linewidth'>>=
...
<<demographics-2, echo=FALSE, fig.width=6.6, fig.height=4, fig.keep='all', fig.show='hold', out.width='\\linewidth'>>=
...
\begin{figure}[ht]
\centering
<<targets, fig.width=6, fig.height=4.0, out.width='0.55\\linewidth', echo=FALSE>>=
...
gol.model <-  glmer(one.correct~plottype + (1|data_name) + (1|individualID), data=modeldata, family=binomial(), control=glmerControl(optimizer="bobyqa"))
gol.fixef <- data.frame(confint(gol.model, method="Wald"))[-(1:2),] # exclude sigmas
names(gol.fixef) <- c("LB", "UB")
gol.fixef$OR <- fixef(gol.model)
gol.fixef[1,1:3] <- 0

suppressMessages(require(multcomp))
type_compare <- glht(gol.model, mcp(plottype="Tukey"))
gol.fixef$letters <- cld(type_compare)$mcletters$Letters
# not sure what to do about those warnings

gol.fixef$label <- gsub("plottype", "", names(fixef(gol.model)))
...
Figure~\ref{fig:onetarget} shows an overview of the number of evaluations by plot type (outlines) and the number of times participants chose at least one of the targets (dark shaded areas). Plot types  associated with  clustering as shown in table~\ref{tab:plotaesthetics} lead to significantly fewer correct evaluations ($\chi^2_{9} = 385$, $p$-value $< 0.0001$). A  possible cause for this is discussed in more detail in section~\ref{sec:sentiment}. 
...
md <- data.frame(plottype = levels(modeldata$plottype), 
                 tendency=c("none", "trend", "group", "group", "group", "group", "conflict", "trend", "group", "conflict"))
...
totals$tendency <- factor(totals$tendency, levels=c("none", "trend", "group", "conflict"))
...
ggplot() + 
  ...
  geom_rect(aes(xmin=0, xmax=eval, ymin=nlabel-.4, ymax=nlabel+0.4, fill=type), 
           data=subset(totals, type=="Correct"), stat="identity", 
           position="identity", colour="black") + 
  scale_fill_manual("# Evaluations", values=c("Correct"="black", "Total" = "grey90")) +
...
<<target-model-vis, dependson='target-model', echo=FALSE, fig.width=9, fig.height=4, out.width='.95\\linewidth'>>=

#yleft <- with(gvl.fixef, LB-OR)[1]
#yright <- with(gvl.fixef, UB-OR)[1]
ggplot(data=gol.fixef) + 
#  geom_segment(aes(x=label, xend=label), y=yleft, yend=yright, 
#               colour="grey80", size=10) +
  geom_hline(yintercept=0, colour="gray70") + 
  geom_pointrange(aes(x=label, y=OR, ymin=LB, ymax=UB), size=.75) + 
  coord_flip() + 
  theme_bw() + 
  ggtitle("Odds of selecting one of the targets") + 
  xlab("") +
  #  xlab("Plot Type") + 
  geom_text(aes(y=-2, x=label, label=letters)) + 
  scale_y_continuous("Odds (on log scale) of identifying one of the targets and 95% Wald Intervals\n(Reference level: Plain plot)", 
                     breaks=c(-2, log(c(1/c(4.5,3, 1.5), 1)), 0.25), labels=c("<--\nMiss", "1/4.5","1/3", "1/1.5", "1", "-->\nIdentification"), limits=c(-2,0.25))

@
\caption{\label{fig:onetarget}I don't think we should include this chart. It shows a bit much of the difference.}
\end{figure}
...
gvl.fixef <- data.frame(confint(gvl.model, method="Wald"))[-(1:2),] # exclude sigmas
...
#               colour="grey80", size=10) +
  ...
  scale_y_continuous("Odds (on log scale) of selecting Cluster over Trend Target and 95% Wald Intervals\n(Reference level: Plain plot)", 
...
\newtext{In summary, the results from this experiment show, that in order to gain a significant difference from a plain representation, and visually emphasize groups or trends, we need to make use of a statistical layer, usually associated with a confidence interval/region, in form of an error band or an ellipse. }
...
\begin{figure}[ht]\centering
<<sentiment, eval=FALSE, echo=FALSE, fig.width=7, fig.height=7, out.width='.3\\linewidth', fig.show='hold'>>=
...
%' group.fixef <- data.frame(summary(group.model)$coefficients, confint(group.model, method="Wald")[-(1:2),])
...
#res <- ldply(1:10, function(x)
#      dVsim(x=0:K, K=K, m=20, scenario=3, target=1:2, N=100000)
#      )
means <- res %>% group_by(x) %>%
  summarize(avg=mean(scenario3))
@
<<twotarget, dependson='twotarget-simu', echo=FALSE, out.width='0.7\\linewidth', fig.width=8, fig.height=5>>=
