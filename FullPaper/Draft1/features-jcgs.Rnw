...
totals <- merge(totals, md, by.x="label", by.y="plottype")
...
md <- merge(md, subset(totals, type=="Correct")[,c("label", "eval")], by.x="plottype", by.y="label")
md <- md[order(md$tendency, md$eval),]

totals$label <- factor(totals$label, levels=md$plottype)
...
ggplot() + 
  ...
  scale_y_continuous(breaks=1:10, labels=md$plottype) +
