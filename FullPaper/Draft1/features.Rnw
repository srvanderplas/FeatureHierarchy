...
\end{itemize}
\newdo{Let's cut back here to all of the rules that are actually needed in the paper, so we kick out common fate, closure and connectedness}
...
\newdo{shouldn't healey be cited here as well?}
