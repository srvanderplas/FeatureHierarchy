...
\section{Simulation based inference in a two-target lineup scenario}
...
\begin{enumerate}
    ...
    \item Pick $K$ values from a Multinomial distribution with $\pi = (1-p)/|| 1- p||$, i.e. $x_j \sim M_\pi, i=1, ..., K$
...
\newdo{Include an example of the simulation's results that's relevant to the paper.}
\end{appendix}
\end{document}