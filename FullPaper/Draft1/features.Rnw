...
\newdo{I think we need to straighten out the analysis: after the overview of the demographics, the first part should be an assessment of the difficulty of the lineup (using visual p-values, I'll get that started), and then do the face-off. 
We might be able to move the single models into a separate paper... so don't delete any of it, I'm just trying to get the flow right at the moment.
}
...
\subsection{Is difficulty of Lineups matching with parameter settings?}
...
res <- ldply(unique(answers2$data_name), function(x){
  return(cbind(data_name = x, read.csv(paste0("../../FeatureHierarchyImages/Turk16/Data/", x, ".csv"))))
})
...
@
\caption{\label{cluster-trend} Overall, the percentages of correct data identifications match variability (i.e.\ they decrease as variability increases). }
\end{figure}
