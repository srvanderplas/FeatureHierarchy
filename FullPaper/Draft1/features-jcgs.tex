...
Data collection was conducted over a NA hour period, 
during which time 1356 individuals completed 13519 unique lineup evaluations. 
Participants who completed fewer than 10 lineups were removed from the study (0 participants, 0 evaluations), and lineup evaluations in excess of 10 for each participant were also removed from the study (13481 evaluations). 
After these data filtration steps, our data consist of 541 trials completed by 10 participants. 

Of the participants who completed at least 10 lineup evaluations, 80\% were male, relatively younger than the US population and relatively well educated (see Figure~\ref{fig:demographics}). 
Each plot was evaluated by between 0 and 2 individuals 
(Mean: 0.02, SD= 0.15).
NA\% of the participant evaluations identified at least one of the two target plots successfully (Trend: NA\%, Cluster: NA\%). 
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in names(clusters) <- c("{}x"{}, "{}Cluster"{}): 'names' attribute [2] must be the same length as the vector [1]}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error: measure variables not found in data: NA}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in do.call("{}layer"{}, list(mapping = mapping, data = data, stat = stat, : object 'clm' not found}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in `[.data.frame`(base, names(rows)): undefined columns selected}}\end{kframe}
\end{knitrout}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in `\$<-.data.frame`(`*tmp*`, "{}OR"{}, value = structure(c(1.16689342958925, : replacement has 10 rows, data has 12}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in `\$<-.data.frame`(`*tmp*`, "{}letters"{}, value = structure(c("{}b"{}, : replacement has 10 rows, data has 12}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in `\$<-.data.frame`(`*tmp*`, "{}label"{}, value = c("{}(Intercept)"{}, "{}trend"{}, : replacement has 10 rows, data has 12}}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'group.fixef' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'group.fixef2' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in eval(expr, envir, enclos): object 'group.fixef2' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in factor(group.fixef2\$label, levels = group.fixef2\$label[order(group.fixef2\$OR, : object 'group.fixef2' not found}}
...
{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in names(group.fixef2)[1:4] <- c("{}Log Odds"{}, "{}Std. Error"{}, "{}Z"{}, : object 'group.fixef2' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in names(group.fixef2)[8] <- "{}Plot Aesthetic"{}: object 'group.fixef2' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in names(group.fixef2)[9] <- "{}Tukey Post Hoc Differences"{}: object 'group.fixef2' not found}}

{\ttfamily\noindent\bfseries\color{errorcolor}{\#\# Error in print(xtable(group.fixef2[, c(8, 1:4)], caption = c("{}Fitted values of fixed effects for the model described in section\textasciitilde{}\textbackslash{}ref\{sec:groupModel\}."{}, : error in evaluating the argument 'x' in selecting a method for function 'print': Error in xtable(group.fixef2[, c(8, 1:4)], caption = c("{}Fitted values of fixed effects for the model described in section\textasciitilde{}\textbackslash{}ref\{sec:groupModel\}."{},\ \ : \\\#\#\ \  object 'group.fixef2' not found}}\end{kframe}
\end{knitrout}
