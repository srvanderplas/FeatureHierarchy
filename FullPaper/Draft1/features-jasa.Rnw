...
\comment{HH: we need an additional sentence here, and I thought we have had one before ... In `this' study - the `this' doesn't make any sense yet, becuse we haven't said that we were doing one. }
...
\newtext{Figure~\ref{fig:onetarget} shows an overview of the number of evaluations by plot type (outlines) and the number of times, participants chose at least one of the targets (shaded areas). Plot types  associated with  clustering as shown in table~\ref{tab:plotaesthetics} seem to lead to (significantly, XXX p-value?) fewer correct evaluations. We will discuss a possible cause for this in more detail in a later section. }
