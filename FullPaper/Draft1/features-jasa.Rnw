...
\title{Clusters beat Trend!? \\Testing feature hierarchy in statistical graphics}
...
\begin{subfigure}[t]{0.28\linewidth}
  ...
  \includegraphics[width=\linewidth]{figure/samplepics-10}
\end{subfigure}
...
<<color-palette, dev='cairo_pdf', echo=FALSE, fig.width=5, fig.height=1, out.width='.5\\linewidth'>>=
colors <- c("#1f77b4", "#ff7f0e", "#2ca02c", "#d62728", "#9467bd", 
            "#8c564b", "#e377c2", "#7f7f7f", "#bcbd22", "#17becf")
...
\done{The following paragraph needs to be updated}
\newtext{For each plot type (aesthetic combination), we consider the probability that a participant selects one of the two target plots, and then we consider the conditional probability of selecting the cluster target (given that the participant has selected one of the two target plots). }
...
\begin{figure}[ht]\centering
<<faceoffdata, echo=FALSE, fig.width=9, fig.height=4.5, out.width='.75\\textwidth'>>=
...
totals <- modeldata %>% group_by(label) %>% summarize(eval=length(label), type="Total")
correct.totals <- faceoff %>% group_by(label) %>% summarize(eval=length(label), type="Correct")
...
ggplot() + 
  ...
  geom_bar(aes(x=label, y=eval, fill=type), data=totals, stat="identity", position="identity", color="black") + 
...
\newtext{In order to assess which of the two stimuli dominated in each of the plot types, we concentrate first on all those responses in which participants identified at least one of the targets (\Sexpr{sum(1-modeldata$neither)} trials). }
\newtext{Figure~\ref{fig:onetarget} shows an overview of the number of evaluations by plot type (outlines) and the number of times, participants chose at least one of the targets (shaded areas). Plot types  associated with  clustering as shown in table~\ref{tab:plotaesthetics} seem to lead to (significantly, XXX p-value?, see section~\ref{sec:sentiment}) fewer correct evaluations. We will discuss a possible cause for this in more detail in a later section. }
...
ggplot(data=gvl.fixef[-1,]) + 
  geom_pointrange(aes(x=label, y=OR, ymin=LB, ymax=UB)) + 
  coord_flip() + 
  theme_bw() + 
  geom_hline(yintercept=0, colour="gray70") + 
  ggtitle("Log Odds of Selecting Cluster vs Trend Target \n (given one target was identified)") + 
  xlab("") +
  #  xlab("Plot Type") + 
  geom_text(aes(y=-1, x=label, label=letters)) + 
  scale_y_continuous("Log Odds (Cluster vs Trend) and 95% Wald Intervals\n(Reference level: Plain plot)", 
                     breaks=c(-1, -0.5, 0, 0.5, 1), labels=c("<--Trend\n  Target", "-0.5", "0.0", "0.5", "Cluster-->\nTarget  "), limits=c(-1,1))
...
\newtext{The lineup experimental protocol allows us to collect participant justifications for their target selection. These short explanations provide some additional insight into participant reasoning, and further support the gestalt explanation for the experimental results. }
...
\begin{figure}
<<twotarget, echo=FALSE, out.width='0.7\\linewidth', fig.width=8, fig.height=5>>=
library(vinference) # install from heike/vinference github
K <- 10
res <- ldply(1:10, function(x)
      dvisual(x=0:K, K=K, m=20, type="scenario3", target=1:2, N=100000)
      )
means <- ddply(res, .(x), summarize, avg=mean(scenario3))
qplot(x, scenario3, data=res) +
theme_bw() + 
ylab("estimated probability") + xlab("Number of times (out of 10) one of the targets is picked") + 
  ...
  geom_point(aes(x=x, y=binom), colour="steelblue", shape=21)
@
\caption{\label{fig:simulation} Ten simulations of size $b_2 = 1,000$ and $b_1 = 100$ for lineups of size $m=20$ assuming $K=10$ evaluations. The averages of the ten simulation runs are shown as lines. The open circles are probabilities from  Binomial $B_{2/20, 10}$.}
\end{figure}
\end{appendix}
\end{document}