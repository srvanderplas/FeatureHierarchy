...
\begin{figure}[ht]\centering
<<faceoffdata, echo=FALSE>>=

faceoff <- subset(modeldata, trend.correct | cluster.correct)
qplot(reorder(plottype,plottype,function(x) -length(x)), data=faceoff) + coord_flip() + theme_bw() + xlab("") + ylab("# of target evaluations")
@
\caption{\label{fig:faceoff} number of evaluations by plot type, in which at least one of the targets was identified. Due to the design each plot type was evaluated almost the same number of times (between 1195 and 1208 times).}
\end{figure}
