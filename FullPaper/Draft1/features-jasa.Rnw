...
\newdo{Include a discussion of how many results we have for each of the plot types. Try to explain differences using parts of the single target models.} 
...
qplot(reorder(plottype,plottype,function(x) -length(x)), data=faceoff) + coord_flip() + theme_bw() + xlab("") + ylab("# of target evaluations") + geom_bar(aes(x=plottype), data=modeldata, fill=NA, colour="black")
@
